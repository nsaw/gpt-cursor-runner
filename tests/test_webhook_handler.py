...
@pytest(.fixture
def client() import app.config["TESTING"] = True
...
# Patch process_hybrid_block to write to tmp_path
    ...
    monkeypatch.setattr(
        webhook_handler,process_hybrid_block","
        lambda data import str(tmp_path / "patch.json"),
    )
    payload = {id"
        "test-id",role" "test-role",patch" {"pattern" "foo", "replacement": "bar"},
    }"
