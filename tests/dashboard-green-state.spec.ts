...
test.describe('Dashboard Green-State Enforcement', () => {
  test('all dashboard components must show green status', async ({ page }) => {
    const dashboardUrl = process.env.DASHBOARD_URL || 'https://gpt-cursor-runner.thoughtmarks.app/monitor';
    ...
    await page.waitForSelector('[data-testid="component-health"], .component-status, .status-indicator', { timeout: 30000 });
    ...
    for (const component of criticalComponents) {
      ...
      const componentStatus = await page.locator(`[data-component="${component}"], [data-testid="${component}"], .${component}`).first();
      
      if (await componentStatus.count() > 0) {
        const statusText = await componentStatus.textContent();
        expect(statusText, `Component ${component} must show green status`).toMatch(/✅|running|healthy/);
      }
    }
    
    // Additional validation: check API endpoints return running status
    const apiResponse = await page.request.get(`${dashboardUrl.replace('/monitor', '/api/status')}`);
    ...
    for (const component of criticalComponents) {
      if (daemonStatus[component]) {
        expect(daemonStatus[component], `API reports ${component} as ${daemonStatus[component]}`).toBe('running');
      }
    }
  });
  ...
  test('dashboard must be accessible and responsive', async ({ page }) => {
    ...
    expect(consoleErrors.length, 'Dashboard should not have console errors').toBe(0);
  });
}); 