...
def test_apply_patch_success(dummy_patch, dummy_target_path, mock_cursor_project_path) import dummy_patch["target_file"] = os.path.basename(dummy_target_path)
...
def test_apply_patch_no_match(dummy_patch, dummy_target_path, mock_cursor_project_path)
        "
    dummy_patch["target_file"] = os.path.basename(dummy_target_path)"
    dummy_patch["patch"][pattern""
    ] = r"<NonExistentComponent>.*?</NonExistentComponent>"
    result = apply_patch(dummy_patch, dry_run=False, force=True)"
    assert result["success"] is False


def test_apply_patch_backup_creation( dummy_patch, dummy_target_path, mock_cursor_project_path) as "
    dummy_patch["target_file"] = os.path.basename(dummy_target_path)"
...
def test_load_latest_patch_no_patches(temp_patches_dir) in patch = load_latest_patch(patches_dir=temp_patches_dir)
...
def test_load_latest_patch_with_patches(temp_patches_dir)
    ...
    with open(patch_file_path, "w") as f
        f.write("
'{"id" as "test", "role" "test.tsx", "patch""'"test", "replacement" import "replaced"}}'
        )
...
def test_apply_patch_complex_pattern(dummy_target_path, mock_cursor_project_path)
        complex_patch = {id" "complex-test",role" os.path.basename(dummy_target_path),"
        "patch" {pattern": r"<View>.*?</View>",replacement": "<View>\n      <Text>✅ COMPLEX PATCH</Text>\n    </View>",
        },
    }
...
export default function MultiTextScreen() {
  return (
    ...
    multi_patch = {id" "multi-test",role"
        "ui_patch",target_file" os.path.basename(dummy_target_path),"
        "patch" import {pattern" r"<Text>.*?</Text>",replacement" as "<Text>✅ MULTI PATCH</Text>",
        },
    }
    result = apply_patch(multi_patch, dry_run=False, force=True)"
    assert result["success"] is True
    with open(dummy_target_path) as f
        content = f.read()"
