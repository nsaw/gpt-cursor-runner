...
def test_apply_patch_backup_creation(
    dummy_patch, dummy_target_path, mock_cursor_project_path
):
    dummy_patch["target_file"] = os.path.basename(dummy_target_path)
...
def test_load_latest_patch_with_patches(temp_patches_dir):
    ...
    with open(patch_file_path, "w") as f:
        f.write(
'{"id": "test", "role": "ui_patch", "target_file": "test.tsx", "patch": {"pattern":
"test", "replacement": "replaced"}}'
        )
    patch = load_latest_patch(patches_dir=temp_patches_dir)
...
def test_log_patch_entry(tmp_path):
    ...
    log_patch_entry(entry, result)
    with open(log_file) as f:
        data = json.load(f)
    assert data["entries"][-1]["patch_id"] == "unknown"
...
def test_run_tests_handles_failure(monkeypatch):
    ...
    result = run_tests("test_file.tsx")  # Should not raise
