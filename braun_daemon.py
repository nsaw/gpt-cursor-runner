...
class BraunDaemon:
    ...
    def run(self):
        ...
        try:
            ...
            print("\n🛑 BRAUN Daemon stopped by user")
        except Exception as e:
            print(f"\n❌ BRAUN Daemon error: {e}")
        finally:
            print(f"\n📊 BRAUN Daemon Summary:")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="BRAUN Daemon for automatic patch processing"
    )
    ...
    daemon = BraunDaemon(patches_dir=patches_dir, check_interval=args.interval)
    daemon.run()
