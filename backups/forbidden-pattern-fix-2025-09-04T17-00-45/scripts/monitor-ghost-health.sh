...
***REMOVED***_LOG="$LOG_DIR/ghost-bridge.log"
...
check_ghost_ack() {
    if grep -q "ACKNOWLEDGED" "$***REMOVED***_LOG" 2>/dev/null; then
...
monitor_logs() {
    ...
    { tail -f "$***REMOVED***_LOG" | grep --line-buffered "ACKNOWLEDGED\|ERROR\|WARN" & } >/dev/null 2>&1 & disown
...
validate_timestamps() {
    ...
    local ghost_mtime=$(stat -f %m "$***REMOVED***_LOG" 2>/dev/null || echo "0")
...
main() {
    ...
    if [ ! -f "$***REMOVED***_LOG" ]; then
        echo "[$(date '+%Y-%m-%d %H:%M:%S')] ❌ Ghost bridge log not found: $***REMOVED***_LOG" | tee -a "$MONITOR_LOG"
        exit 1
    fi
