...
def fix_unterminated_strings_final(content: str) -> str:
    ...
    content = re.sub(r'return\s+"""([^"]*)$', r'return """\1"""', 
                     content, flags=re.MULTILINE)
    content = re.sub(r"return\s+'''([^']*)$", r"return '''\1'''", 
                     content, flags=re.MULTILINE)
    ...
    content = re.sub(r'"""([^"]*)\s([^"]*)$', r'"""\1\s\2"""', 
                     content, flags=re.MULTILINE)
    content = re.sub(r"'''([^']*)\s([^']*)$", r"'''\1\s\2'''", 
                     content, flags=re.MULTILINE)
    
    # Fix unterminated strings with special characters
    content = re.sub(r'"""([^"]*)[^\w\s"]([^"]*)$', r'"""\1[^\w\s"]\2"""', 
                     content, flags=re.MULTILINE)
    content = re.sub(r"'''([^']*)[^\w\s']([^']*)$", r"'''\1[^\w\s']\2'''", 
                     content, flags=re.MULTILINE)
...
def main():
    ...
    os.system("python3 -m flake8 . --count --select=E9,F63,F7,F82 --show-source --statistics | tail -5")
