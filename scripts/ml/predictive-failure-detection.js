...
class PredictiveFailureDetection {
  ...
  ensureDirectories() {
    [this.dataDir, this.modelsDir, this.featuresDir, this.predictionsDir].forEach(dir => {
  ...
  async initializeFeatureExtractor() {
    ...
    this.featureExtractor = {
      ...
      extractProcessHealth: async () => {
        ...
        // Ghost processes
        const ghostProcesses = ['ghost-bridge', 'patch-executor', 'summary-monitor'];
      ...
      extractPatchFeatures: async () => {
        ...
        try {
          ...
          const pendingPatches = fs.readdirSync(patchesDir).filter(f => f.endsWith('.json')).length;
          features.pendingPatches = pendingPatches;
          
          // Recent patch success rate
          const summariesDir = this.config.dataSources.summaries;
          const recentSummaries = fs.readdirSync(summariesDir)
            .filter(f => f.endsWith('.md'))
            .slice(-10);
          
          let successCount = 0;
          for (const summary of recentSummaries) {
            const content = fs.readFileSync(path.join(summariesDir, summary), 'utf8');
            if (content.includes('✅ PASS') || content.includes('Status: ✅')) {
              successCount++;
            }
          }
          features.recentSuccessRate = recentSummaries.length > 0 ? successCount / recentSummaries.length : 1;
          ...
          for (const logFile of logFiles.slice(-5)) { // Last 5 log files
            ...
            for (const line of lines) {
              if (line.toLowerCase().includes('error')) {
                ...
                if (line.toLowerCase().includes('connection')) connectionErrors++;
                if (line.toLowerCase().includes('validation')) validationErrors++;
              }
            }
          }
  ...
  async initializeModelManager() {
    ...
    this.modelManager = {
      ...
      loadModels: async () => {
        const modelFiles = fs.readdirSync(this.modelsDir).filter(f => f.endsWith('.json'));
      ...
      createDefaultModels: async () => {
        const defaultModels = {
          ...
          processHealth: {
            name: 'processHealth',
            type: 'ensemble',
            features: ['ghost-bridge_running', 'patch-executor_running', 'summary-monitor_running'],
            weights: { 'ghost-bridge_running': 0.4, 'patch-executor_running': 0.4, 'summary-monitor_running': 0.2 },
            threshold: 0.8
          },
          patchExecution: {
            name: 'patchExecution',
            type: 'neural',
            features: ['pendingPatches', 'recentSuccessRate', 'avgExecutionTime'],
            weights: { pendingPatches: 0.3, recentSuccessRate: 0.5, avgExecutionTime: 0.2 },
            threshold: 0.75
          },
          errorPatterns: {
            name: 'errorPatterns',
            type: 'statistical',
            features: ['totalErrors', 'timeoutErrors', 'connectionErrors', 'validationErrors'],
            weights: { totalErrors: 0.3, timeoutErrors: 0.3, connectionErrors: 0.2, validationErrors: 0.2 },
            threshold: 0.6
          }
        };
      ...
      retrainModel: async (name, model) => {
        ...
        const recentPerformance = await this.calculateModelPerformance(name, historicalData);
  ...
  async initializePredictionEngine() {
    ...
    this.predictionEngine = {
      ...
      predict: async (features) => {
        ...
        for (const [name, model] of Object.entries(this.modelManager.models)) {
          const prediction = await this.predictionEngine.predictWithModel(name, model, features);
          predictions[name] = prediction;
        }
        
        // Calculate ensemble prediction
        predictions.ensemble = this.predictionEngine.calculateEnsemblePrediction(predictions);
      ...
      calculateEnsemblePrediction: (predictions) => {
        ...
        return {
          score: avgScore,
          confidence: this.predictionEngine.calculateConfidence(avgScore, { threshold: 0.7 }),
  ...
  async saveFeatures(features) {
    ...
    const featureFile = path.join(this.featuresDir, `features-${timestamp}.json`);
  ...
  async savePredictions(predictions) {
    const timestamp = new Date().toISOString().replace(/[:.]/g, '-');
    const predictionFile = path.join(this.predictionsDir, `predictions-${timestamp}.json`);
  ...
  async checkAlerts(predictions) {
    ...
    for (const [name, prediction] of Object.entries(predictions)) {
      if (prediction.risk !== 'safe' && prediction.confidence >= this.config.confidenceThreshold) {
        alerts.push({
          model: name,
  ...
  calculateAverageExecutionTime() {
    try {
      const summariesDir = this.config.dataSources.summaries;
      const summaries = fs.readdirSync(summariesDir)
        .filter(f => f.endsWith('.md'))
        .slice(-20);
