...
update_env_files() {
    ...
    for env_file in "${env_files[@]}"; do
        ...
        if [ -f "$full_path" ]; then
            ...
            echo "***REMOVED***_URL=https://$***REMOVED***_HOSTNAME" >> "$full_path"
...
update_script_files() {
    ...
    find "$repo_path" -name "*.sh" -type f | while read -r script_file; do
        ...
        sed -i '' "s|ghost\.thoughtmarks\.app|$***REMOVED***_HOSTNAME|g" "$script_file"
    ...
    find "$repo_path" -name "*.js" -type f | while read -r js_file; do
        ...
        sed -i '' "s|ghost\.thoughtmarks\.app|$***REMOVED***_HOSTNAME|g" "$js_file"
    ...
    find "$repo_path" -name "*.py" -type f | while read -r py_file; do
        ...
        sed -i '' "s|ghost\.thoughtmarks\.app|$***REMOVED***_HOSTNAME|g" "$py_file"
...
update_doc_files() {
    ...
    find "$repo_path" -name "*.md" -type f | while read -r md_file; do
        ...
        sed -i '' "s|ghost\.thoughtmarks\.app|$***REMOVED***_HOSTNAME|g" "$md_file"
...
| Ghost Runner | $***REMOVED***_HOSTNAME | $***REMOVED***_TUNNEL_ID | 5556 | Secondary/backup runner |
