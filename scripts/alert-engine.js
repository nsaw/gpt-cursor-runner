...
const ALERTS_FILE = path.join(CYOPS_CACHE, 'telemetry', 'alert-engine-state.json');
...
class AlertEngine {
  ...
  updateSummary() {
    this.alerts.summary = {
      ...
      errorCount: this.alerts.active.filter(a => a.severity === 'error').length,
  ...
  async start() {
    ...
    setTimeout(() => {
      this.addAlert('warning', 'High CPU usage detected on system', 'system-monitor');
