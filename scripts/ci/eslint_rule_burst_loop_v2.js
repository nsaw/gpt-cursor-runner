...
const PREFIXER=path.join(ROOT,"scripts/ci/eslint_apply_unused_prefix_once.js");
...
const pass=(args,out)=>new Promise(res=>{
  ...
  const t=setTimeout(()=>{try{p.kill('SIGKILL');}catch(_){}res({code:124})},900000); p.on('exit',c=>{clearTimeout(t);res({code:c??1})});
});
const tsc=()=>new Promise(res=>{ const p=spawn(process.execPath,[path.join(ROOT,"scripts/ci/run_tsc_once.js")],{cwd:ROOT,stdio:'inherit'}); p.on('exit',c=>res(c??1)); });
const sum=(file)=>{ try{ const r=JSON.parse(fs.readFileSync(file,'utf8')); let e=0,w=0; const files=[];
  for(const f of r){ const msgs=f.messages||[]; const errs=msgs.filter(m=>m.severity===2).length; const warns=msgs.filter(m=>m.severity===1).length;
    if(errs||warns) files.push({file:f.filePath,errs,warns}); e+=errs; w+=warns; } return {errors:e,warnings:w,files}; }catch{ return {errors:-1,warnings:-1,files:[]}; } };
...
(async()=>{
  // Pre & offenders & inventory (ensure exists)
  ...
  if(!fs.existsSync(RULES)){ const raw=JSON.parse(fs.readFileSync(PRE,'utf8')); const tally={};
  ...
  // Sample dry-run on top 30 files
  const sample=pre.files.sort((a,b)=>b.errs-a.errs).slice(0,30).map(f=>f.file);
  fs.writeFileSync(SAMPLE_LOG,"sample_count="+sample.length+"\n");
  if(sample.length){
    await new Promise(res=>{ const p=spawn(process.execPath,[RUNNER,...sample,"--fix-dry-run","--max-warnings","1000000"],{cwd:ROOT,stdio:'inherit'});
      const t=setTimeout(()=>{ try{p.kill('SIGKILL');}catch(_){}res(); },180000); p.on('exit',()=>{clearTimeout(t);res();}); });
  }
  let iter=0;
  while(iter<12){
    iter++;
    // Rule burst for top 8 fixable rules
    for(const r of rules.slice(0,8)){ await pass(["--fix","--fix-type","problem,suggestion,layout","--max-warnings","0",...force(r)], null); }
    // Offender chunk pass (expand window)
    const ranked=JSON.parse(fs.readFileSync(OFF,'utf8')).ranked||[];
    const dirs=ranked.slice(0, Math.min(12+iter*2, 36)).map(x=>x.dir);
    for(const d of dirs){ await pass(["--fix","--fix-type","problem,suggestion,layout","--max-warnings","0", d], null); }
    // Compile
    await tsc();
    // Measure
    await pass(["-f","json","-o",POST,"--max-warnings","1000000"], null);
    const post=sum(POST);
    fs.writeFileSync(DELTA, JSON.stringify({iter, pre:{e:pre.errors,w:pre.warnings}, post:{e:post.errors,w:post.warnings}, dE:pre.errors-post.errors, dW:pre.warnings-post.warnings}, null, 2));
    fs.writeFileSync(OFF, JSON.stringify({ ranked: offendersFrom(post.files) }, null, 2));
    // Plateau? try unused-prefixer once per plateau
    const plateau = (pre.errors-post.errors)<=0 && (pre.warnings-post.warnings)<=0;
    if(plateau){
      const px=spawn(process.execPath,[PREFIXER],{cwd:ROOT,stdio:'inherit'}); await new Promise(r=>px.on('exit',()=>r()));
      await pass(["--fix","--fix-type","problem,suggestion,layout","--max-warnings","0"], null);
      await tsc();
      await pass(["-f","json","-o",POST,"--max-warnings","1000000"], null);
    }
    pre = sum(POST);
    if(pre.errors===0 && pre.warnings<=20) break;
  }
  const final=sum(POST);
  process.exit(final.errors===0 && final.warnings<=20 ? 0 : 2);
})();
