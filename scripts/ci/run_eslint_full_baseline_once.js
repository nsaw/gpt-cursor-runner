...
(async () => {
  ...
  const OUT =
    '/Users/sawyer/gitSync/.cursor-cache/ROOT/.logs/eslint-full-baseline.json';
  const CWD = '/Users/sawyer/gitSync/gpt-cursor-runner';
  const qPath =
    '/Users/sawyer/gitSync/gpt-cursor-runner/config/eslint.quarantine.manifest.json';
  ...
  const globs = (man.quarantine || [])
    .map((g) => g.replace(/\*\*/g, '/**/*'))
    .map((g) => (g.endsWith('/**') ? `${g}/*.{js,ts,tsx}` : g));
  ...
  try {
    ...
    console.log(
      JSON.stringify(
        { ok: false, counts: { errors, warnings }, files: results.length },
        null,
        2,
      ),
    );
