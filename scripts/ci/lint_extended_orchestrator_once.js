...
#!/usr/bin/env node
// lint_extended_orchestrator_once.js (resume-capable)
...
const budgetMs = Number(process.argv[2]||"5400000");
const cooldownMs = Number(process.argv[3]||"2000");
...
async function codemod(rules){ const logOut="/Users/sawyer/gitSync/.cursor-cache/ROOT/.logs/codemod-disable-log.json"; return runNode([DISABLE_CM, ESLINT_JSON, rules.join(','), "40", logOut]); }
function checkpoint(){ fs.writeFileSync(ORCH_SUM, JSON.stringify(state,null,2)); }
process.on('SIGINT', ()=>{ log("[orchestrator] SIGINT, checkpoint and exit 130"); checkpoint(); process.exit(130); });
process.on('SIGTERM',()=>{ log("[orchestrator] SIGTERM, checkpoint and exit 130"); checkpoint(); process.exit(130); });
(async()=>{
  ...
  while (Date.now()-start < budgetMs){
    ...
    // Always try codemod with preferred rules (bounded)
