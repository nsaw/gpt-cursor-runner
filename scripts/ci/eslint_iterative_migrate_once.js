...
(async () => {
  ...
  function shouldAllowDisable(p) {
    ...
    if (
      rel.startsWith('scripts/g2o') ||
      rel.startsWith('scripts/ci') ||
      rel.startsWith('scripts/metrics')
    )
      return false;
    if (
      rel.startsWith('scripts/ghost') ||
      rel.startsWith('src-nextgen/ghost') ||
  ...
  if (e1 === 0 && w1 <= 20) {
    fs.writeFileSync(
      OUT_PASS2,
      JSON.stringify(
        { pass: 2, skipped: true, reason: 'threshold met' },
        null,
        2,
      ),
    );
    fs.writeFileSync(OUT_TOP, JSON.stringify({ files: [] }, null, 2));
    console.log(
      JSON.stringify({ ok: true, pass: 1, errors: e1, warnings: w1 }, null, 2),
    );
    process.exit(0);
  }
  ...
  const s2 = {
    ...
    errors: e2,
    warnings: w2,
    top: summarize(res2).slice(0, 50),
    disabled,
  };
  fs.writeFileSync(OUT_PASS2, JSON.stringify(s2, null, 2));
  console.log(
    JSON.stringify(
      { ok: e2 === 0 && w2 <= 20, pass: 2, errors: e2, warnings: w2 },
      null,
      2,
    ),
  );
