...
const runMarathonIteration = async () => {
  ...
  fix();
  lint();
  const {errors: errorsAfterFix, warnings: warningsAfterFix} = parseEslintResults();
  runHotspots();
  
  update({ok:(errorsAfterFix===0&&warningsAfterFix<20),pass,phase:'post-fix',errors:errorsAfterFix,warnings:warningsAfterFix,elapsed_ms:Date.now()-started});
