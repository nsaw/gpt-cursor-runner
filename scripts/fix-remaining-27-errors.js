...
function fixRemainingLogEventIssues(content) {
  ...
  content = content.replace(/this\.logEvent\s*\(\s*['"]system_startup['"],\s*['"]info['"]\s*\)/g, 
    'this.logEvent(\'system_startup\', \'System started\', \'info\')');
  
  // Fix component_error calls with valid event types for each file
  content = content.replace(/this\.logEvent\s*\(\s*['"]component_error['"],\s*['"]([^'"]+)['"],\s*['"]([^'"]+)['"]\s*\)/g, 
    'this.logEvent(\'error\', \'Component error detected\', \'$2\')');
  
  // Fix logEvent calls with too many arguments for specific APIs
  content = content.replace(/this\.logEvent\s*\(\s*['"](config_error|state_error)['"],\s*([^,]+),\s*['"]([^'"]+)['"]\s*\)/g, 
    'this.logEvent(\'$1\', $2)');
  
  // Fix logEvent calls with invalid event types in orchestrator
  content = content.replace(/this\.logEvent\s*\(\s*['"](config_error|state_error|dashboard_error|monitoring_error|system_error)['"],\s*([^,]+),\s*['"]([^'"]+)['"]\s*\)/g, 
