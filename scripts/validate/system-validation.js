...
const CONFIG = {
  ...
  PATHS: {
    ...
    UNIFIED_MONITOR_LOG: '/Users/sawyer/gitSync/gpt-cursor-runner/logs/unified-monitor.log',
    UNIFIED_MONITOR_HEARTBEAT: '/Users/sawyer/gitSync/.cursor-cache/CYOPS/.heartbeat/.unified-monitor.json',
...
function loadAllEndpoints() {
  try {
    if (fs.existsSync(CONFIG.PATHS.TUNNELS_FILE)) {
      const tunnelsData = JSON.parse(fs.readFileSync(CONFIG.PATHS.TUNNELS_FILE, 'utf8'));
      const endpoints = [];
...
class SystemValidator {
  ...
  async validateMonitoring() {
    ...
    try {
      ...
      const logContent = logExists ? fs.readFileSync(CONFIG.PATHS.UNIFIED_MONITOR_LOG, 'utf8') : '';
      const hasRecentActivity = logContent.includes(new Date().toISOString().split('T')[0]);
    ...
    try {
      const heartbeatExists = fs.existsSync(CONFIG.PATHS.UNIFIED_MONITOR_HEARTBEAT);
      let heartbeatData = null;
      
      if (heartbeatExists) {
        heartbeatData = JSON.parse(fs.readFileSync(CONFIG.PATHS.UNIFIED_MONITOR_HEARTBEAT, 'utf8'));
      }
      
      const isRecent = heartbeatData && 
        (new Date() - new Date(heartbeatData.timestamp)) < 60000; // Within last minute
  ...
  async validateHealth() {
    ...
    for (const endpoint of this.endpoints) {
      try {
        ...
        this.log(`  ${endpoint.name} (${endpoint.type}): FAIL - ${error.message}`);
      }
    }
  }
  ...
  async runValidation() {
    ...
    this.log(`📊 Validating ${this.endpoints.length} endpoints from TUNNELS.json`);
    this.log('');
    ...
    this.log(`  Pass Rate: ${this.results.summary.passCount}/${this.results.summary.totalCount} (${this.results.summary.passRate}%)`);
    this.log('');
  ...
  generateReport() {
    ...
    const reportPath = '/Users/sawyer/gitSync/gpt-cursor-runner/logs/system-validation.json';
