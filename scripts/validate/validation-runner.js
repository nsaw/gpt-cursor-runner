...
class ValidationRunner {
  ...
  async runFullValidation() {
    ...
    try {
      ...
      this.validationResults.commandValidation = await this.validator.validateDirectory('./scripts');
      
      // Phase 2: Runtime Validation
      console.log('\n🔧 Phase 2: Runtime Validation');
      this.validationResults.runtimeValidation = await this.runRuntimeValidation();
  ...
  async runRuntimeValidation() {
    const checks = [
      { name: 'Ghost Runner Health', command: 'curl -s -m 5 http://localhost:5051/health' },
      { name: 'Patch Executor Status', command: 'ps aux | grep "patch-executor" | grep -v grep' },
      { name: 'Daemon Processes', command: 'ps aux | grep "consolidated-daemon" | grep -v grep' },
      { name: 'Monitoring Scripts', command: 'ps aux | grep "real-dual_monitor" | grep -v grep' }
    ];
    ...
    console.log(`✅ Runtime checks passed: ${results.passed}/${results.passed + results.failed}`);
    return results;
  }

  async executeCheck(check) {
    return new Promise((resolve) => {
      exec(check.command, { timeout: 10000 }, (error, stdout, _stderr) => {
        if (error) {
          resolve({
            name: check.name,
            status: 'FAIL',
  ...
  async runComplianceCheck() {
    const complianceRules = [
      { name: 'Non-blocking patterns enforced', check: () => this.checkNonBlockingPatterns() },
      { name: 'Validation gates active', check: () => this.checkValidationGates() },
      { name: 'Monitoring systems active', check: () => this.checkMonitoringSystems() },
      { name: 'Error handling implemented', check: () => this.checkErrorHandling() }
    ];
    ...
    console.log(`✅ Compliance checks passed: ${results.passed}/${results.passed + results.failed}`);
    return results;
  }
  ...
  determineOverallStatus() {
    ...
    const complianceValid = this.validationResults.complianceCheck?.failed === 0;
  ...
  async generateValidationReport() {
    ...
    if (this.validationResults.overallStatus === 'FAIL') {
      console.log('❌ Critical issues detected. Please fix blocking patterns before proceeding.');
      process.exit(1);
