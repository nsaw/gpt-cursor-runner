...
async function testIncomingWebhook() {
  try {
    ...
    const response = await axios.post(SLACK_CONFIG.incomingWebhook, testMessage, {
      headers: {
        'Content-Type': 'application/json'
      }
    });
...
async function testWebhookServer() {
  try {
    ...
    console.log(`   - Error: ${error.message}`);
    console.log('   - Please start the server with: node webhook-thoughtmarks-server.js');
    return false;
  }
}
...
async function testTunnelConnectivity() {
  try {
    const tunnelUrl = 'https://webhook-thoughtmarks.thoughtmarks.app/health';
...
async function runAllTests() {
  ...
  console.log(`Bot Token Validation: ${results.botToken ? '✅ PASS' : '❌ FAIL'}`);
  ...
  if (passedTests === totalTests) {
    ...
    console.log('\n🔧 Troubleshooting:');
    if (!results.envVars) console.log('- Check config/webhook-thoughtmarks.env');
