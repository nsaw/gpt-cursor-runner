...
app.get(_'/slack/oauth/callback', _async (req, res) => {;
...
;
    ...
    const _tokenResponse = await axios.post(';'';
...
;
// Update environment file with new token;
async function updateEnvironmentFile(_accessToken) {;
  try {;
    ...
    let _envContent = fs.readFileSync(envPath, 'utf8');
;
    ...
    console.log('[INSTALL] ✅ Updated environment file with new bot token')} catch (_error) {';'';
    console.error('[INSTALL] Error updating environment file:', error.message)}};

// Test the installation';'';
app.get(_'/test', _async (req, res) => {;
  if (!installationState.accessToken) {;
    return res.json({';'';
  ...
  try {;
    ...
    const _testResponse = await axios.post(';'';
...
;
    if (testResponse.data.ok) {;
      ...
      res.json({ error: 'Token test failed', details: testResponse.data })}} catch (_error) {';'';
