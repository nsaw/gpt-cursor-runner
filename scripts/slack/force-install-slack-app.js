...
async function checkAppInfo() {';'';
  ...
  try {';'';
    const _response = await axios.get('https://slack.com/api/apps.info', {;
      headers: {'`;
        Authorization: `Bearer ${SLACK_APP_TOKEN}`,'';
        'Content-Type': 'application/json',
      },
...
async function main() {';'';
  ...
  console.log('');
;
  const _appTokenValid = await testAppToken()';'';
  console.log('');
;
  const _appInfo = await checkAppInfo()';'';
  console.log('');
;
  const _isInstalled = await getInstallationInfo()';'';
  console.log('');
;
  const _messageSent = await testAppMessage()';'';
  console.log('');
';'';
...
;
  if (appTokenValid && appInfo && isInstalled && messageSent) {';'';
    ...
    console.log('   3. The webhook-thoughtmarks server should work')} else {';'';
