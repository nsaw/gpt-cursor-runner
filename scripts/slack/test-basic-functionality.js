...
async function testServerHealth() {
  ...
  try {
    const response = await axios.get('http://localhost:5432/health', { timeout: 5000 });
...
async function testTunnelConnectivity() {
  ...
  try {
    const response = await axios.get('https://webhook-thoughtmarks.thoughtmarks.app/health', { timeout: 10000 });
    if (response.status === 200) {
...
async function testIncomingWebhook() {
  ...
  try {
    const response = await axios.post(SLACK_INCOMING_WEBHOOK, {
...
async function testSlashCommandEndpoint() {
  ...
  try {
    const response = await axios.post('http://localhost:5432/slack/commands', {
      command: '/test',
      text: 'test',
      user_id: 'U1234567890',
...
async function testOAuthCallback() {
  ...
  try {
    const response = await axios.get('http://localhost:5432/slack/oauth/callback', { timeout: 5000 });
    if (response.status === 400) {
      console.log('   ✅ OAuth callback endpoint working (400 expected for no code)');
      return true;
    } else {
...
async function main() {
  ...
  const tunnelConnectivity = await testTunnelConnectivity();
  console.log('');
  
  const incomingWebhook = await testIncomingWebhook();
  console.log('');
  
  const slashCommand = await testSlashCommandEndpoint();
  console.log('');
  
  const oauthCallback = await testOAuthCallback();
  console.log('');
  ...
  const workingComponents = [serverHealth, tunnelConnectivity, incomingWebhook, slashCommand, oauthCallback].filter(Boolean).length;
  ...
  if (workingComponents >= 4) {
    ...
    console.log('   3. Basic functionality is operational');
    ...
    if (!oauthCallback) console.log('   • OAuth callback has issues');
  } else {
