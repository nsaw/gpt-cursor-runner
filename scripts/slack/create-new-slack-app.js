...
const APP_CONFIG = {
  ...
  long_description: 'Robots using robots to control robots. This Slack app serves as a command interface for a GPT-powered hybrid automation pipeline.',
  redirect_urls: ['https://webhook-thoughtmarks.thoughtmarks.app/slack/oauth/callback'],
  scopes: {
...
async function createSlackApp() {
  ...
  try {
    ...
    const createResponse = await axios.post(`${SLACK_API_BASE}/apps.create`, {
    ...
    const oauthResponse = await axios.post(`${SLACK_API_BASE}/apps.oauth.config.update`, {
      app_id: appId,
    ...
    for (const cmd of APP_CONFIG.slash_commands) {
      try {
        const cmdResponse = await axios.post(`${SLACK_API_BASE}/apps.commands.create`, {
          app_id: appId,
          command: cmd.command,
          url: cmd.url,
        ...
        if (cmdResponse.data.ok) {
          ...
          console.log(`   ⚠️  Failed to add ${cmd.command}: ${cmdResponse.data.error}`);
        }
      } catch (error) {
...
`;
    ...
    const envPath = path.join(__dirname, '..', 'config', 'webhook-thoughtmarks-v2.env');
