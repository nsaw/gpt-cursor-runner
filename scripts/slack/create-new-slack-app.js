...
;
  try {;
    ...
    const _createResponse = await axios.post(;
...
;
    ...
    const _appId = createResponse.data.app.id;
    const _clientId = createResponse.data.app.client_id;
    const _clientSecret = createResponse.data.app.client_secret;
    const _appToken = createResponse.data.app.app_token;
';'';
...
;
    ...
    const _oauthResponse = await axios.post(`;
...
;
    if (!oauthResponse.data.ok) {';'';
      console.log('⚠️  OAuth config update failed, but continuing...')} else {';'';
    ...
    for (const cmd of APP_CONFIG.slash_commands) {;
      try {;
        const _cmdResponse = await axios.post(`;
...
;
        if (cmdResponse.data.ok) {`;
          console.log(`   ✅ Added ${cmd.command}`)} else {;
          console.log(`;
    ...
    const _envContent = `# New Slack App Credentials;
...
;
    ...
    const _scopes = APP_CONFIG.scopes.bot.join(',')`;
    const _oauthUrl = `https://slack.com/oauth/v2/authorize?client_id=${clientId}&scope=${scopes}&redirect_uri=${encodeURIComponent(APP_CONFIG.redirect_urls[0])}&state=new-app-install-${Date.now()}`;
';'';
