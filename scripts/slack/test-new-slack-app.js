...
async function testNewSlackApp() {
  ...
  try {
    ...
    const authResponse = await axios.post('https://slack.com/api/auth.test', null, {
      headers: {
    ...
    if (authResponse.data.ok) {
      ...
      console.log(`   ❌ Bot token validation failed: ${authResponse.data.error}`);
    }
    ...
    const appResponse = await axios.post('https://slack.com/api/apps.connections.open', null, {
      headers: {
    ...
    if (appResponse.data.ok) {
      ...
      console.log(`   ❌ App token validation failed: ${appResponse.data.error}`);
    }

    // Test 4: Generate OAuth URL
    console.log('\n🔗 Test 4: Generating OAuth URL...');
    const scopes = ['commands', 'chat:write', 'incoming-webhook', 'assistant:write'].join(',');
    ...
    if (authResponse.data.ok && messageResponse.data.ok && appResponse.data.ok) {
