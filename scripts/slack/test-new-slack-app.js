...
const _SLACK_CLIENT_ID = process.env.SLACK_CLIENT_ID;
;
...
;
  try {;
    ...
    const _authResponse = await axios.post(';'';
...
;
    if (authResponse.data.ok) {';'';
      ...
      console.log(`   👤 Bot user: ${authResponse.data.user}`)} else {;
      console.log(`;
    ...
    const _messageResponse = await axios.post(';'';
...
;
    if (messageResponse.data.ok) {';'';
      ...
      console.log(`   📨 Message ID: ${messageResponse.data.ts}`)} else {;
      console.log(`;
    ...
    const _appResponse = await axios.post(';'';
...
;
    if (appResponse.data.ok) {';'';
      console.log('   ✅ App token is valid')} else {;
      console.log(`;
    ...
    const _oauthUrl = `https://slack.com/oauth/v2/authorize?client_id=${SLACK_CLIENT_ID}&scope=${scopes}&redirect_uri=${encodeURIComponent('https://webhook-thoughtmarks.thoughtmarks.app/slack/oauth/callback')}&state=test-new-app-${Date.now()}`;
';'';
...
;
    if (;
      ...
      console.log('')';'';
      console.log('🚀 Ready to proceed with installation!')} else {';'';
      console.log('⚠️  Some tests failed')';'';
      console.log('   Please check the configuration and try again')}} catch (_error) {';'';
