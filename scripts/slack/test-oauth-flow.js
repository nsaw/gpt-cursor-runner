...
console.log(`   Client Secret: ${SLACK_CLIENT_SECRET ? '✅ Set' : '❌ Missing'}`);
...
try {
  const oauthUrl = `https://slack.com/oauth/v2/authorize?client_id=${SLACK_CLIENT_ID}&scope=commands,chat:write,users:read,app_mentions:read,incoming-webhook,channels:history&redirect_uri=${encodeURIComponent(REDIRECT_URI)}`;
...
try {
  const response = await axios.get('https://webhook-thoughtmarks.thoughtmarks.app/slack/oauth/callback', {
    timeout: 5000
  });
  console.log(`   ✅ Callback endpoint accessible (Status: ${response.status})`);
...
try {
  const response = await axios.get('http://localhost:5432/slack/oauth/callback', {
    timeout: 5000
  });
  console.log(`   ✅ Local callback endpoint accessible (Status: ${response.status})`);
} catch (error) {
...
const freshOAuthUrl = `https://slack.com/oauth/v2/authorize?client_id=${SLACK_CLIENT_ID}&scope=commands,chat:write,users:read,app_mentions:read,incoming-webhook,channels:history&redirect_uri=${encodeURIComponent(REDIRECT_URI)}&state=debug-test-${Date.now()}`;
...
console.log('- If you see "invalid_code" error, the code expired or was already used');
console.log('- If you see "redirect_uri_mismatch", check the redirect URL in Slack app settings');
console.log('- If you see "invalid_client", check the client ID and secret');
console.log('- If you see "missing_scope", check the requested scopes vs. configured scopes');
}
