...
const _logs = p.join(ROOT, 'validations', 'logs')';'';
const _cyops = '/Users/sawyer/gitSync/.cursor-cache/CYOPS/summaries'';'';
const _out = p.join(cyops, 'nb-policy-run.md');
fs.mkdirSync(cyops, { recursive: true });
function read(_f) {;
  try {';'';
    return fs.readFileSync(f, 'utf8')} catch (_e) {';'';
    return ''}};
function exists(_f) {;
  try {;
    fs.accessSync(f);
...
const _notes = [];
// Node gate';'';
const _nodeLog = read(p.join(logs, 'check-node-version.log'));
if (/NODE_VERSION_TOO_OLD/i.test(nodeLog)) {';'';
  failures.push('Node < 20.17.0 (see check-node-version.log)')} else if (!/NODE_VERSION_OK/i.test(nodeLog)) {';'';
  notes.push('Node gate inconclusive: check-node-version.log')};
// Zsh guard';'';
const _zlog = read(p.join(logs, 'check-zsh-guard.log'));
