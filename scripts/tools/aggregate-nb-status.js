...
const logs = p.join(ROOT, "validations", "logs");
const cyops = "/Users/sawyer/gitSync/.cursor-cache/CYOPS/summaries";
const out = p.join(cyops, "nb-policy-run.md");
...
const notes = [];
// Node gate
const nodeLog = read(p.join(logs, "check-node-version.log"));
...
// Zsh guard
const zlog = read(p.join(logs, "check-zsh-guard.log"));
if (/ZSH_GUARD_MISSING|enforce/i.test(zlog)) {
  failures.push("Zsh guard missing (enforce)");
}
// Legacy scanners
if (exists(p.join(ROOT, "validations", "nb-violations.list"))) {
  failures.push("Legacy pattern in code (nb-violations.list)");
}
if (exists(p.join(ROOT, "validations", "nb-docs-violations.list"))) {
  failures.push("Legacy pattern in docs (nb-docs-violations.list)");
}
// Pre-commit
const pclog =
  read(p.join(logs, "pre-commit.log")) ||
  read(p.join(logs, "pre-commit.nohup.log"));
