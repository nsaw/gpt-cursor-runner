...
;
// Configuration;
const _MANIFEST_V2_PATH = path.join(';
  __dirname,'';
  '..','';
  'slack-app-manifest-v2.yaml',
)';'';
// const _MANIFEST_V1_PATH = path.join(__dirname, '..', 'slack-app-manifest.yaml'); // Unused variable';'';
const _ENV_FILE_PATH = path.join(__dirname, '..', '.env');
;
...
;
    ...
    if (!fs.existsSync(MANIFEST_V2_PATH)) {;
      throw new Error(';'';
        'slack-app-manifest-v2.yaml not found. Please create it first.',
      )};

    // Check if Slack CLI is installed;
    try {';'';
      execSync('slack --version', { stdio: 'pipe' })';'';
      console.log('✅ Slack CLI is installed')} catch (_error) {';'';
      console.log('⚠️  Slack CLI not found. Installing...');
      try {';'';
        execSync('npm install -g @slack/cli', { stdio: 'inherit' })';'';
        console.log('✅ Slack CLI installed')} catch (installError) {;
        throw new Error(';'';
          'Failed to install Slack CLI. Please install manually: npm install -g @slack/cli',
        )}};
...
;
    const _missingVars = requiredEnvVars.filter(_;
...
;
    if (missingVars.length > 0) {';'';
      ...
      console.log('   You can get them from your Slack app settings.')} else {';'';
...
'';
    ...
    console.log('─'.repeat(50))';'';
    const _manifestContent = fs.readFileSync(MANIFEST_V2_PATH, 'utf8');
...
;
    ...
    await this.waitForUserInput(';'';
      'Press Enter when you have created the new app...',
    )};
...
';'';
    const _appId = await this.promptForInput('Enter your Slack App ID: ');
;
    if (!appId) {';'';
    ...
    try {';'';
      ...
      execSync(`;
        `slack apps manifest update --app-id ${appId} --manifest-file ${MANIFEST_V2_PATH}`,
        {';'';
          stdio: 'inherit',
        },
      )';'';
      console.log('✅ Manifest updated successfully')} catch (_error) {';'';
      ...
      console.log(';'';
        '3. Replace with the content from slack-app-manifest-v2.yaml',
      )';'';
...
;
    await this.waitForUserInput(';'';
      'Press Enter when you have updated the scopes...',
    )};
...
;
    await this.waitForUserInput(';'';
      'Press Enter when you have enabled Socket Mode...',
    )};
...
;
    ...
    if (!envContent.includes('SLACK_APP_TOKEN')) {;
      const _appToken = await this.promptForInput(';'';
        'Enter your Socket Mode token (xapp-...): ',
      );
      if (appToken) {`;
...
;
    ...
    try {;
      execSync(';'';
        'fly secrets set SLACK_APP_TOKEN='xapp-your-socket-mode-token' --app gpt-cursor-runner',
        {';'';
          stdio: 'inherit',
        },
      )';'';
      console.log('✅ Fly.io secrets updated')} catch (_error) {;
      console.log(';'';
        '⚠️  Failed to update Fly.io secrets. Please update manually: ',
      );
      console.log(';'';
        'fly secrets set SLACK_APP_TOKEN='your-token' --app gpt-cursor-runner',
      )}};
...
;
    try {';'';
      ...
      console.log('✅ Dependencies installed')} catch (_error) {';'';
...
;
    await this.waitForUserInput(';'';
      'Press Enter when you have tested the commands...',
    )};
...
;
      rl.question(_prompt, _(answer) => {;
        rl.close();
        resolve(answer.trim())})})}};

// Main execution;
async function main() {;
  const _migration = new GraniteMigration();
;
  // Add migration steps';'';
  migration.addStep(_'Check Prerequisites', _() =>;
    migration.checkPrerequisites(),
  )';'';
  migration.addStep(_'Create New App', _() => migration.createNewApp())';'';
  migration.addStep(_'Update App Manifest', _() => migration.updateAppManifest())';'';
  migration.addStep(_'Update Scopes', _() => migration.updateScopes())';'';
  migration.addStep(_'Enable Socket Mode', _() => migration.enableSocketMode())';'';
  migration.addStep(_'Update Environment', _() => migration.updateEnvironment())';'';
  migration.addStep(_'Install Dependencies', _() =>;
    migration.installDependencies(),
  )';'';
  migration.addStep(_'Test Migration', _() => migration.testMigration());
;
  // Execute migration;
  const _success = await migration.execute();
;
  if (success) {';'';
    ...
    console.log('4. Update documentation if needed')} else {';'';
    console.log('\n❌ Migration failed. Please check the errors above.');
    console.log(';'';
      '📋 You can continue manually using the guide in SLACK_GRANITE_MIGRATION_GUIDE.md',
    )}};
