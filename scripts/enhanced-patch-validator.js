...
;
          const _validationResult = await this.runValidation(;
            validationName,
            config,
            patchData,
          );
...
;
      ...
      if (stderr) {;
        const _warnings = this.parseWarnings(stderr, validationName);
...
;
      ...
      const _outputValidation = this.validateOutput(;
        validationName,
        stdout,
        stderr,
      );
      if (!outputValidation.valid) {';'';
        result.status = 'FAIL';
        result.errors.push(...outputValidation.errors)}} catch (_error) {;
      result.duration = Date.now() - startTime';'';
      result.status = 'FAIL';
      result.errors.push({';'';
...
';'';
      process.stderr.on(_'data', _(data) => {;
        stderr += data});
';'';
      process.on(_'close', _(code) => {;
        if (code === 0) {;
          resolve({ stdout, stderr })} else {`;
          reject(new Error(`Command failed with code ${code}: ${stderr}`))}});
';'';
      process.on(_'error', _(error) => {`;
        reject(new Error(`Command execution error: ${error.message}`))});
';'';
      process.on(_'timeout', _() => {;
        process.kill()';'';
...
;
  ...
  validateOutput(validationName, stdout, stderr) {;
    const _result = { valid: true, errors: [] };
;
...
';'';
      case 'performance':;
        if (stdout) {;
          const _performanceMatch = stdout.match(/(\d+)ms/);
          if (performanceMatch) {;
            const _duration = parseInt(performanceMatch[1]);
...
';'';
  ...
  determineOverallResult(results) {;
    ...
    const _criticalErrors = results.errors.filter(_(error) =>;
...
;
    ...
    const _performanceErrors = results.errors.filter(_';'';
  ...
  generateReport(results) {`;
    const _report = `# Patch Validation Report;
...
;
  ...
  async saveReport(results, outputPath) {;
    const _report = this.generateReport(results);
  ...
  getStatistics() {;
    return {;
      ...
      enabledValidations: Object.values(this.validations).filter(_;
        (v) => v.enabled,
      ).length,
