...
const _CONFIG = {';'';
  ...
  ***REMOVED***_STATUS_FILES: [';'';
  ...
  VALIDATION_RULES: {';'';
    ...
    ***REMOVED***_TRACE_FIELDS: ['ghostStatus', 'ghostUptime', 'ghostLastCheck'],
    FORMAT_RULES: {;
      PATCH_NAME_PATTERN:;
...
'';
      ...
      this.log('PARSER_INIT', 'Summary Ghost Parser initialized successfully')} catch (_error) {';'';
  ...
  async loadGhostStatus() {;
    for (const statusFile of CONFIG.***REMOVED***_STATUS_FILES) {;
      try {;
        if (fs.existsSync(statusFile)) {';'';
          const _statusData = JSON.parse(fs.readFileSync(statusFile, 'utf8'));
          const _systemKey = path.basename(path.dirname(statusFile));
          this.ghostStatus[systemKey] = statusData`;
          console.log(`👻 [PARSER] Loaded ghost status for ${systemKey}`)}} catch (_error) {;
        console.warn(`;
...
;
      ...
      const _metadata = this.extractMetadata(content, filename);
;
      // Validate the summary;
      const _validation = this.validateSummary(metadata, content, filename);
;
      return {;
        file: filename,
        path: filePath,
...
;
    ...
    const _patchNameMatch = filename.match(/^summary-(.+)\.md$/);
    if (patchNameMatch) {;
      metadata.patchName = patchNameMatch[1]};

    // Enhanced status extraction with failure marker detection;
    const _statusMatch = content.match(/Status[:\s]*([A-Z_]+)/i);
    if (statusMatch) {;
...
;
      ...
      const _ghostStatusMatches = content.matchAll(;
        CONFIG.VALIDATION_RULES.TRACE_DECODERS.***REMOVED***_STATUS,
      );
      ...
      const _uptimeMatches = content.matchAll(;
        CONFIG.VALIDATION_RULES.TRACE_DECODERS.***REMOVED***_UPTIME,
      );
      ...
      const _lastCheckMatches = content.matchAll(;
        CONFIG.VALIDATION_RULES.TRACE_DECODERS.***REMOVED***_LAST_CHECK,
      );
...
;
  ...
  extractTimestamp(content) {;
    for (const pattern of CONFIG.VALIDATION_RULES.TRACE_DECODERS;
      .TIMESTAMP_VARIANTS) {;
      const _match = content.match(pattern);
      if (match) {;
        return match[1]}};
    return null};
  ...
  extractMarkdownLinks(content) {;
    const _links = [];
    const _matches = content.matchAll(;
...
;
    ...
    if (!metadata.patchName) {';'';
      errors.push('Missing patch name')} else if (;
    ...
    if (!metadata.status) {';'';
      warnings.push('Missing status field')} else if (;
    ...
    if (metadata.ghostStatus) {;
      const _systemKey = this.determineSystemKey(filename);
      if (this.ghostStatus[systemKey]) {;
        const _actualStatus = this.ghostStatus[systemKey].status;
    ...
    for (const summaryDir of CONFIG.SUMMARY_DIRS) {;
      try {;
        ...
        const _files = fs;
          ...
          .filter(_(file) => file.endsWith('.md'))';'';
          .filter(_(file) => file.startsWith('summary-'));
;
        console.log(`;
...
;
        for (const file of files) {;
          const _filePath = path.join(summaryDir, file);
          const _result = this.parseSummaryFile(filePath);
;
...
';'';
    let _report = '# Summary Ghost Parser Validation Report\n\n'`;
...
;
    ...
    if (summary.errors.length > 0) {';'';
      ...
      summary.errors.forEach(_(error) => {';''`;
    ...
    results.forEach(_(result) => {';'';
      const _status = result.validation.valid ? "✅' : '❌';
      const _score = result.validation.score`;
...
;
  ...
  log(level, message) {;
    try {;
      ...
      const _logEntry = `[${timestamp}] [${level}] ${message}\n`;
      fs.appendFileSync(CONFIG.LOG_FILE, logEntry)} catch (_error) {';'';
  ...
  exportResults(format = 'json') {;
    ...
    const _exportPath = `/Users/sawyer/gitSync/gpt-cursor-runner/logs/summary-validation-${Date.now()}.${format}`;
;
    try {';'';
      if (format === 'json') {;
        fs.writeFileSync(;
      ...
      return exportPath} catch (_error) {';'';
...
;
    ...
    const _results = await this.parseAllSummaries();
';'';
...
if (require.main === module) {;
  const _parser = new SummaryGhostParser();
';'';
  const _command = process.argv[2] || 'validate';
;
  switch (command) {';'';
    case 'validate':;
      parser;
...
';'';
    case 'export':';'';
      const _format = process.argv[3] || 'json';
      parser;
        .runValidation();
        .then(_() => {;
          const _exportPath = parser.exportResults(format);
          if (exportPath) {`;
            console.log(`✅ [PARSER] Results exported to: ${exportPath}`)};
          process.exit(0)});
        .catch(_(error) => {';'';
