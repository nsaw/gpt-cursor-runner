...
# Update ghost status
echo '{"status": "STARTING", "timestamp": "'$(date -u +%Y-%m-%dT%H:%M:%SZ)'", "services": {"ghost-bridge": "'$***REMOVED***_BRIDGE_PID'", "patch-executor": "'$PATCH_EXECUTOR_PID'", "summary-monitor": "'$SUMMARY_MONITOR_PID'"}}' > summaries/_heartbeat/.ghost-status.json
...
if kill -0 $***REMOVED***_BRIDGE_PID 2>/dev/null && kill -0 $PATCH_EXECUTOR_PID 2>/dev/null && kill -0 $SUMMARY_MONITOR_PID 2>/dev/null; then
    echo '{"status": "LIVE", "timestamp": "'$(date -u +%Y-%m-%dT%H:%M:%SZ)'", "services": {"ghost-bridge": "'$***REMOVED***_BRIDGE_PID'", "patch-executor": "'$PATCH_EXECUTOR_PID'", "summary-monitor": "'$SUMMARY_MONITOR_PID'"}}' > summaries/_heartbeat/.ghost-status.json
