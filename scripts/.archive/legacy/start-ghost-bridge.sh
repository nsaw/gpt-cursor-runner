...
echo "$(date -u +%Y-%m-%dT%H:%M:%SZ) - Ghost bridge started (PID: $***REMOVED***_BRIDGE_PID)" >> summaries/_heartbeat/.last-md-write.log

# Wait a moment for service to initialize
sleep 3

# Check if service is running
if kill -0 $***REMOVED***_BRIDGE_PID 2>/dev/null; then
    echo "$(date -u +%Y-%m-%dT%H:%M:%SZ) - Ghost bridge LIVE (PID: $***REMOVED***_BRIDGE_PID)" >> summaries/_heartbeat/.last-md-write.log
    echo "✅ Ghost bridge is LIVE (PID: $***REMOVED***_BRIDGE_PID)"
    exit 0
else
