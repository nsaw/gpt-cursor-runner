...
app.get('/monitor', (req, res) => {
  try {
    ...
    const registry = fs.existsSync(registryFile) ? JSON.parse(fs.readFileSync(registryFile)) : {};
    const status = fs.existsSync(statusPath) ? JSON.parse(fs.readFileSync(statusPath)) : {};
...
app.get('/status', (req, res) => {
  try {
    ...
    const registry = fs.existsSync(registryFile) ? JSON.parse(fs.readFileSync(registryFile)) : {};
    const diagnostics = fs.existsSync(diagFile) ? JSON.parse(fs.readFileSync(diagFile)) : {};
    const status = fs.existsSync(statusPath) ? JSON.parse(fs.readFileSync(statusPath)) : {};
