...
const _ValidationRunner = require('../validate/validation-runner')';'';
const _AgentTraining = require('../compliance/agent-training');
;
...
;
    setTimeout(_async () => {;
      if (this.monitoringActive) {;
        await this.runValidationCycle();
        this.scheduleNextRun()}}, this.monitoringInterval)};
...
;
    try {;
      ...
      const _validationRunner = new ValidationRunner();
      const _results = await validationRunner.runFullValidation();
;
...
;
    ...
    if (results.commandValidation?.failed > 0) {;
      violations.push(_{';'';
        type: 'COMMAND_PATTERN_VIOLATION', _';'';
        severity: 'HIGH', _;
        count: results.commandValidation.failed, _;
    ...
    if (results.runtimeValidation?.failed > 0) {;
      violations.push(_{';'';
        type: 'RUNTIME_VIOLATION', _';'';
        severity: 'MEDIUM', _;
        count: results.runtimeValidation.failed, _;
    ...
    if (results.complianceCheck?.failed > 0) {;
      violations.push(_{';'';
        type: 'COMPLIANCE_VIOLATION', _';'';
        severity: 'HIGH', _;
        count: results.complianceCheck.failed, _;
...
;
    ...
    this.violationLog = this.violationLog.filter(_;
...
;
    // Save alert';'';
    const _alertDir = './logs/alerts';
    await fs.mkdir(alertDir, { recursive: true });
`;
    const _alertFile = path.join(alertDir, `alert-${Date.now()}.json`);
...
';'';
    const _reportDir = './logs/monitoring';
    await fs.mkdir(reportDir, { recursive: true });
;
    const _reportFile = path.join(`;
      reportDir,
...
`;
  ...
  groupViolationsByType() {;
    const _grouped = {};
    this.violationLog.forEach(_(violation) => {;
...
;
    ...
    const _training = new AgentTraining();
    const _complianceReport = await training.generateComplianceReport();
;
...
if (require.main === module) {;
  const _monitor = new ComplianceMonitor();
;
