...
function updateIndex(root) {
  try {
    ...
    if (fs.existsSync(patchesDir)) {
      const mainPatches = fs.readdirSync(patchesDir).filter(f => f.endsWith('.json'));
    ...
    if (fs.existsSync(patchesArchiveDir)) {
      const archivedPatches = fs.readdirSync(patchesArchiveDir).filter(f => f.endsWith('.json'));
    ...
    if (fs.existsSync(patchesFailedDir)) {
      const failedPatches = fs.readdirSync(patchesFailedDir).filter(f => f.endsWith('.json'));
    ...
    if (fs.existsSync(summariesCompletedDir)) {
      const completedSummaries = fs.readdirSync(summariesCompletedDir).filter(f => f.endsWith('.json'));
    ...
    if (fs.existsSync(summariesArchiveDir)) {
      const archivedSummaries = fs.readdirSync(summariesArchiveDir).filter(f => f.endsWith('.json'));
    ...
    if (fs.existsSync(summariesFailedDir)) {
      const failedSummaries = fs.readdirSync(summariesFailedDir).filter(f => f.endsWith('.json'));
