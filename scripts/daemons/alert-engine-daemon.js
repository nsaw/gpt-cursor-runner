...
const LOG_FILE = path.join(LOG_DIR, 'alert-engine-daemon.log');
const PID_FILE = path.join(PID_DIR, 'alert-engine-daemon.pid');
...
class AlertEngineDaemon {
  ...
  async start() {
    ...
    const alertEnginePath = path.join(__dirname, 'alert-engine.js');
    ...
    try {
      ...
      this.process.on('exit', (code, signal) => {
        this.isRunning = false;
        this.log(`Alert engine process exited with code ${code} and signal ${signal}`);
        
        if (code !== 0 && this.restartCount < this.maxRestarts) {
          this.restartCount++;
          this.log(`Restarting alert engine (attempt ${this.restartCount}/${this.maxRestarts})...`);
      ...
      this.process.on('error', (error) => {
        this.log(`Alert engine process error: ${error.message}`, 'error');
      });
...
const daemon = new AlertEngineDaemon();
