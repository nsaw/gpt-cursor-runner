...
const LOG_FILE = path.join(LOG_DIR, 'autonomous-decision-daemon.log');
const PID_FILE = path.join(PID_DIR, 'autonomous-decision-daemon.pid');
...
class AutonomousDecisionDaemon {
  ...
  async start() {
    ...
    const enginePath = path.join(__dirname, '../../src-nextgen/ghost/autonomy/autonomousDecisionEngineRunner.ts');
    ...
    try {
      ...
      this.process.on('exit', (code, signal) => {
        this.isRunning = false;
        this.log(`Engine process exited with code ${code} and signal ${signal}`);
        
        if (code !== 0 && this.restartCount < this.maxRestarts) {
          this.restartCount++;
          this.log(`Restarting engine (attempt ${this.restartCount}/${this.maxRestarts})...`);
      ...
      this.process.on('error', (error) => {
        this.log(`Engine process error: ${error.message}`, 'error');
      });
...
const daemon = new AutonomousDecisionDaemon();
