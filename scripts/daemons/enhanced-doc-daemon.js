...
const CONFIG = {
  ...
  logFile: '/Users/sawyer/gitSync/gpt-cursor-runner/logs/enhanced-doc-daemon.log'
};
...
function moveToArchive(filePath, archiveDir) {
  try {
    ...
    log(`Archived: ${fileName} -> ${path.relative(process.cwd(), finalArchivePath)}`);
    return true;
  } catch (error) {
...
function organizeSummaries(root) {
  ...
  files.forEach(file => {
    if (file.startsWith('.') || file === '.archive' || file === '.completed' || file === '.failed') {
...
function generatePatchManifest(dirPath, manifestPath) {
  try {
    ...
    const manifest = {
      ...
      directory: dirPath,
      summary: {
        total: patches.length + completed.length + failed.length + archived.length,
      ...
      failed: failed,
      archived: archived,
      changelog: generateChangelog([...patches, ...completed, ...failed, ...archived])
    };
    
    // Write manifest
    fs.writeFileSync(manifestPath, JSON.stringify(manifest, null, 2));
    log(`Generated patch manifest: ${path.relative(process.cwd(), manifestPath)}`);
...
function generateChangelog(allPatches) {
  ...
  const sortedPatches = allPatches.sort((a, b) => new Date(b.modified) - new Date(a.modified));
    ...
    if (fs.existsSync(dirPath)) {
      ...
      if (files.length > 0) {
        ...
        files.forEach(file => {
          const ext = path.extname(file);
          const size = fs.statSync(path.join(dirPath, file)).size;
          const modified = fs.statSync(path.join(dirPath, file)).mtime.toISOString();
...
function processDirectoryRecursively(dirPath, depth = 0) {
  ...
  try {
    ...
    if (path.basename(dirPath) === 'patches') {
      const manifestPath = path.join(path.dirname(dirPath), 'patch-manifest.json');
