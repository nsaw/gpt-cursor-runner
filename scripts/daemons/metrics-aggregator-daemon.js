...
class MetricsAggregatorDaemon {
  ...
  async start() {
    ...
    const aggregatorPath = path.join(__dirname, '../../src-nextgen/ghost/telemetry/ghostMetricsAggregatorRunner.ts');
    ...
    try {
      ...
      this.process.on('exit', (code, signal) => {
        this.isRunning = false;
        this.log(`Aggregator process exited with code ${code} and signal ${signal}`);
        
        if (code !== 0 && this.restartCount < this.maxRestarts) {
          this.restartCount++;
          this.log(`Restarting aggregator (attempt ${this.restartCount}/${this.maxRestarts})...`);
