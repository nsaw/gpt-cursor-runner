...
class RealAlertEngine {
  ...
  log(message, level = 'info') {
    ...
    console.log(`[${timestamp}] [${level.toUpperCase()}] [AlertEngine] ${message}`);
  }
  ...
  updateSummary() {
    this.alerts.summary = {
      ...
      criticalCount: this.alerts.active.filter(a => a.severity === 'critical').length,
      warningCount: this.alerts.active.filter(a => a.severity === 'warning').length,
  ...
  async checkSystemHealth() {
    try {
      ...
      if (diskUsage > 90) {
        this.addAlert('critical', `Disk usage critical: ${diskUsage}%`, 'storage');
      ...
      if (memoryUsage > 90) {
        this.addAlert('critical', `Memory usage critical: ${memoryUsage}%`, 'system');
      } else if (memoryUsage > 80) {
        this.addAlert('warning', `Memory usage high: ${memoryUsage}%`, 'system');
      }
  ...
  async getMemoryUsage() {
    return new Promise((resolve) => {
      exec('top -l 1 | grep PhysMem | awk \'{print $2}\' | sed \'s/%//\'', (error, stdout) => {
        if (error) {
          resolve(0);
        } else {
          resolve(parseInt(stdout.trim()) || 0);
  ...
  async checkFailedPatches() {
    try {
      ...
      if (fs.existsSync(failedPatchesDir)) {
        const failedPatches = fs.readdirSync(failedPatchesDir)
          .filter(f => f.endsWith('.json'))
          .length;
        
        if (failedPatches > 10) {
          this.addAlert('warning', `${failedPatches} patches have failed`, 'patch-executor');
        }
      }
    } catch (error) {
  ...
  async checkStuckProcesses() {
    try {
      ...
      const processes = ['ghost-bridge-simple.js', 'dual-monitor-server.js', 'gpt_cursor_runner.main'];
      
      for (const processName of processes) {
        const uptime = await this.getProcessUptime(processName);
        if (uptime > 3600) { // More than 1 hour
          this.addAlert('info', `${processName} has been running for ${Math.floor(uptime/3600)}h`, 'process-monitor');
        }
      }
    } catch (error) {
  ...
  async getProcessUptime(processName) {
    return new Promise((resolve) => {
      exec(`ps -eo pid,etime,comm | grep "${processName}" | head -1 | awk '{print $2}'`, (error, stdout) => {
  ...
  async start() {
    ...
    // Add startup alert
    this.addAlert('info', 'Real Alert Engine started successfully', 'alert-engine');
    ...
    if (this.config.monitoring.enabled) {
      ...
      this.log(`System monitoring started with ${this.config.monitoring.intervalMs}ms interval`);
    }
