...
async function writeSummary(_content, _filename) {;
  const _agent = autoDetectAgent();
;
  try {';'';
...
;
    // Route the summary;
    const _result = await routeSummary(content, filename, agent);
;
...
// Update agent status;
async function updateAgentStatus(_status) {;
  const _agent = autoDetectAgent();
