...
';'';
const _GHOST_RELAY_URL = process.env.GHOST_RELAY_URL || 'http://localhost:3001';
;
...
'';
  ...
  try {;
    const _response = await axios.post(`${GHOST_RELAY_URL}/summary/${agent}`, {;
      content,
      filename,
    });
`;
...
async function updateStatus(_agent, _status) {;
  try {`;
    const _response = await axios.post(`${GHOST_RELAY_URL}/status/${agent}`, {;
      status,
    });
`;
...
async function getStatus() {;
  try {`;
    const _response = await axios.get(`${GHOST_RELAY_URL}/status`);
    return response.data} catch (_error) {';'';
