...
echo "[***REMOVED***-RELAY] Starting ghost relay system..."
...
{ pm2 start ecosystem.config.js --only ghost-relay & } >/dev/null 2>&1 & disown || {
  echo "[***REMOVED***-RELAY] Starting ghost relay directly (non-blocking)..."
...
if curl -s http://localhost:3001/health | grep -q "healthy"; then
  echo "[***REMOVED***-RELAY] ✅ Ghost relay started successfully"
else
  echo "[***REMOVED***-RELAY] ❌ Ghost relay failed to start"
  exit 1
fi
...
echo "[***REMOVED***-RELAY] Ghost relay system ready"
echo "[***REMOVED***-RELAY] API: http://localhost:3001"
echo "[***REMOVED***-RELAY] Health: http://localhost:3001/health"
echo "[***REMOVED***-RELAY] Status: http://localhost:3001/status" 
