...
const _***REMOVED***_RELAY_URL = 'http: //localhost:3001'';'';
const _***REMOVED***_VIEWER_URL = 'http://localhost:7474'';'';
...
const _COMPONENTS = {;
  ghostRelay: {';'';
    name: 'ghost-relay',
    port: 3001,
    healthUrl: `${***REMOVED***_RELAY_URL}/health`,'';
    pm2Name: 'ghost-relay','';
  ...
  ghostViewer: {';''`;
    name: 'ghost-viewer',
    port: 7474,
    healthUrl: `${***REMOVED***_VIEWER_URL}/ghost`,'';
...
`;
    const _alertMessage = `[***REMOVED***-DAEMON] ${level.toUpperCase()}: ${component} - ${message}`;
...
;
        ...
        return result.stdout.includes('***REMOVED*** STATUS')} catch (curlError) {;
        return false}}};
...
class GhostUnifiedDaemon {;
  ...
  async start() {';'';
    console.log('[***REMOVED***-DAEMON] Starting unified daemon...');
    this.isRunning = true;
;
...
';'';
    console.log('[***REMOVED***-DAEMON] Unified daemon started successfully');
;
  ...
  async ensureComponentsRunning() {';'';
    console.log('[***REMOVED***-DAEMON] Ensuring all components are running...');
;
...
;
  ...
  async shutdown() {';'';
    console.log('[***REMOVED***-DAEMON] Shutting down unified daemon...');
    this.isRunning = false;
;
...
if (require.main === module) {;
  ...
  daemon.start().catch(_(error) => {';''";
    console.error('[***REMOVED***-DAEMON] Failed to start daemon:", error);
    process.exit(1)})};
