...
;
  try {;
    ...
    console.log(`[***REMOVED***-RELAY] Updated ${agent} status: ${status}`)} catch (_error) {;
    console.error(`;
      `[***REMOVED***-RELAY] Error updating ${agent} status: `,
      error.message,
    )}};
...
const _app = express();
const _PORT = process.env.***REMOVED***_RELAY_PORT || 3001;
;
...
'';
  ...
  try {;
    ...
    console.error(`;
      `[***REMOVED***-RELAY] Error writing summary for ${agent}:`,
      error.message,
    )';'';
...
// Start server;
app.listen(_PORT, _() => {`;
  console.log(`[***REMOVED***-RELAY] Relay server running on port ${PORT}`)';''";
  console.log('[***REMOVED***-RELAY] Monitoring CYOPS and MAIN agents");
  console.log(`;
    `[***REMOVED***-RELAY] Status logs: ${CYOPS_ROOT}/.logs/ghost-relay.log, ${MAIN_ROOT}/.logs/ghost-relay.log`,
  )});
;
