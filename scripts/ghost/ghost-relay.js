...
[path.join(CYOPS_ROOT, '.logs'), path.join(MAIN_ROOT, '.logs')].forEach(logDir => {
...
function initializeLogFiles() {
  ...
  fs.writeFileSync(path.join(CYOPS_ROOT, '.logs', 'ghost-relay.log'), initialStatus);
  fs.writeFileSync(path.join(MAIN_ROOT, '.logs', 'ghost-relay.log'), initialStatus);
}
...
app.get('/status', (req, res) => {
  const cyopsStatus = tryRead(path.join(CYOPS_ROOT, '.logs', 'ghost-relay.log'));
...
app.listen(PORT, () => {
  ...
  console.log(`[GHOST-RELAY] Status logs: ${CYOPS_ROOT}/.logs/ghost-relay.log, ${MAIN_ROOT}/.logs/ghost-relay.log`);
});
