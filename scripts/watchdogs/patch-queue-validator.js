...
function checkQueueHealth() {
  const patchFiles = getFileTimes(patchesPath).filter(f => f.name.endsWith('.json'));
  const summaryFiles = getFileTimes(summariesPath).filter(f => f.name.endsWith('.md'));
  const recentSummary = summaryFiles.sort((a, b) => b.mtime - a.mtime)[0];
  const stalePatches = patchFiles.filter(p => Date.now() - p.mtime > 15 * 60 * 1000);
