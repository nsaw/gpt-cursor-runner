...
start_bridge() {
    ...
    { pkill -f "ghost-bridge-simple.js" & } >/dev/null 2>&1 & disown || true
    sleep 2
    
    # Start bridge
    cd "$PROJECT_ROOT"
    { nohup node scripts/ghost-bridge-simple.js > "$LOG_DIR/ghost-bridge.log" 2>&1 & } >/dev/null 2>&1 & disown
    local bridge_pid="$(pgrep -f 'ghost-bridge-simple.js' | head -1)"
...
restart_bridge() {
    ...
    if [ -f "$BRIDGE_PID_FILE" ]; then
        local pid=$(cat "$BRIDGE_PID_FILE")
        { kill "$pid" 2>/dev/null || true & } >/dev/null 2>&1 & disown || true
        rm -f "$BRIDGE_PID_FILE"
    fi
    
    { pkill -f "ghost-bridge-simple.js" 2>/dev/null || true & } >/dev/null 2>&1 & disown || true
    sleep 3
