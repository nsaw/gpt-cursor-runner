...
LOG='/Users/sawyer/gitSync/.cursor-cache/CYOPS/.logs/p0_queue_shape.log'
ORDER=(
...
: > "$LOG"
plain_list=()
for bn in "${ORDER[@]}"; do [ -f "$ROOTQ/$bn" ] && plain_list+=("$bn"); done
if [ ${#plain_list[@]} -le 1 ]; then exit 0; fi
keep="${plain_list[0]}"
for ((i=1;i<${#plain_list[@]};i++)); do mv -f "$ROOTQ/${plain_list[$i]}" "$ROOTQ/${plain_list[$i]}.hold"; echo "$(date -u +%FT%T.%3NZ) REHOLD ${plain_list[$i]} (keeping $keep)" >> "$LOG"; done
exit 0
