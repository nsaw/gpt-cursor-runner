...
// Load configuration
require('dotenv').config({ path: '/Users/sawyer/gitSync/.cursor-cache/CYOPS/config/dashboard.env' });

const URL = process.env.DASHBOARD_URL;
const TOKEN = process.env.DASHBOARD_TOKEN;
const CF_API_TOKEN = process.env.CF_API_TOKEN;
const LOOP_LOG = '/Users/sawyer/gitSync/gpt-cursor-runner/logs/patch-executor-loop.log';
...
function heartbeat() {
  try {
    ...
    let rss = 0;
    try {
      const rssOutput = execSync("ps -o rss= -p $(pgrep -f patch_executor_daemon.py) || echo 0").toString().trim();
    ...
    let queue = 0;
    try {
      queue = fs.readdirSync(PATCH_DIR).filter(f => f.endsWith('.json')).length;
    } catch (e) {
    ...
    let summaries = 0;
    try {
      summaries = fs.readdirSync('/Users/sawyer/gitSync/.cursor-cache/CYOPS/summaries/').filter(f => f.endsWith('.md')).length;
    } catch (e) {
...
function tailLog() {
  try {
    ...
    const lines = fs.readFileSync(LOOP_LOG, 'utf-8').split('\n').slice(-100).join('\n');
    post({
      type: 'log',
