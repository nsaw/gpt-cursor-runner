...
;
exec(_';'';
  'pkill -f cloudflared && nohup cloudflared tunnel run ghost-thoughtmarks > logs/tunnel-restart.log 2>&1 & disown', _;
  (err) => {;
    if (err) {`;
      safeLog(`❌ Tunnel restart failed: ${err.message}`)} else {';'';
