...
const CRITICAL_PROCESSES = {
  CYOPS: [
    { name: 'ghost-bridge', pattern: 'ghost-bridge.js', script: 'scripts/hooks/ghost-bridge.js', required: true },
    { name: 'heartbeat-loop', pattern: 'heartbeat-loop.js', script: 'scripts/watchdog/heartbeat-loop.js', required: true },
    { name: 'doc-daemon', pattern: 'doc-daemon.js', script: 'scripts/daemons/doc-daemon.js', required: true },
    { name: 'patch-executor', pattern: 'patch-executor.js', script: 'scripts/patch-executor.js', required: false },
    { name: 'summary-monitor', pattern: 'summary-monitor.js', script: 'scripts/hooks/summary-monitor.js', required: false }
  ],
  MAIN: [
    { name: 'expo-dev-server', pattern: 'expo', script: 'npx expo start --clear', required: false },
    { name: 'patch-executor', pattern: 'patch-executor.js', script: 'scripts/patch-executor.js', required: false }
  ]
};
...
function loadTunnels() {
  try {
    if (fs.existsSync(CONFIG.TUNNELS_FILE)) {
      const tunnelsData = JSON.parse(fs.readFileSync(CONFIG.TUNNELS_FILE, 'utf8'));
      const tunnels = [];
  ...
  return [
    { name: 'cloudflare-tunnel', url: 'https://gpt-cursor-runner.fly.dev/health', type: 'cloudflare', status: 'ACTIVE' },
    { name: 'ngrok-tunnel', url: 'https://runner.thoughtmarks.app/health', type: 'ngrok', status: 'ACTIVE' }
  ];
}
...
class UnifiedSystemMonitor {
  ...
  async checkTunnelHealth(tunnel) {
    try {
      ...
      const isHealthy = stdout.includes('healthy') || stdout.includes('ok') || stdout.length > 0;
  ...
  async checkResourceUsage() {
    try {
      ...
      return {
        ...
        disk: diskUsage,
        cpu: cpuUsage,
        healthy: memoryUsage < CONFIG.MEMORY_THRESHOLD && 
  ...
  async restartProcess(processName, scriptPath, systemName) {
    ...
    if (restartCount >= CONFIG.MAX_RESTART_ATTEMPTS) {
      this.log('error', `Process ${processName} exceeded max restart attempts`, {
        processName,
        restartCount,
    ...
    if (now - lastRestart < CONFIG.RESTART_COOLDOWN) {
      this.log('warn', `Process ${processName} restart skipped due to cooldown`, {
        processName,
    ...
    try {
      this.log('info', `Restarting process ${processName}`, { processName, scriptPath });
      ...
      const { stdout, stderr } = await execAsync(`cd "${SYSTEMS[systemName].root}" && node "${fullScriptPath}"`, {
        detached: true,
        stdio: 'ignore'
      });
  ...
  async monitorProcesses() {
    for (const [systemName, system] of Object.entries(SYSTEMS)) {
      ...
      for (const process of processes) {
        const isHealthy = await this.checkProcessHealth(process.name, process.pattern);
        
        if (!isHealthy) {
          if (process.required) {
            this.log('warn', `Required process ${process.name} is down`, { systemName, processName: process.name });
            
            // Attempt restart for required processes
            const restartSuccess = await this.restartProcess(process.name, process.script, systemName);
            
            if (!restartSuccess) {
              this.log('error', `Failed to restart required process ${process.name}`, { systemName, processName: process.name });
            }
          } else {
            this.log('debug', `Optional process ${process.name} is down`, { systemName, processName: process.name });
          }
        } else {
          // Reset restart count if process is healthy
          if (this.restartCounts[process.name] > 0) {
            this.log('info', `Process ${process.name} recovered`, { systemName, processName: process.name });
  ...
  start() {
    ...
    this.log('info', `Loaded ${this.tunnels.length} tunnels/endpoints from TUNNELS.json`);
    this.isRunning = true;
...
default:
  ...
  console.log('');
  console.log('Usage: node unified-system-monitor.js [start|stop|status|health]');
  console.log('');
  ...
  console.log('');
  console.log('This monitor consolidates all system monitoring into a single,');
