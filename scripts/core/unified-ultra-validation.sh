...
validate_ghost_status() { local s="$ROOT/scripts/validate-ghost-status.sh"; if [ -x "$s" ]; then log "[GHOST]"; (cd "$ROOT" && run_with_timeout 45 "bash scripts/validate-ghost-status.sh"); else log "[GHOST] skipped"; fi }
