...
main() {
    ...
    # Check port bindings
    if ! check_port $***REMOVED***_BRIDGE_PORT "Ghost Bridge"; then
        all_healthy=false
    fi
    ...
    if ! check_port $***REMOVED***_RELAY_PORT "Ghost Relay"; then
        all_healthy=false
    fi
    ...
    if ! check_health "$***REMOVED***_BRIDGE_HEALTH" "Ghost Bridge Health"; then
        all_healthy=false
    fi
