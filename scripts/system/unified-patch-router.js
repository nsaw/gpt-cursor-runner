...
function startPatchExecutorIfNeeded(target) {
  try {
    ...
    const result = execSync('ps aux | grep "patch-executor" | grep -v grep | wc -l', { encoding: 'utf8' });
...
function healthCheck() {
  ...
  try {
    const { execSync } = require('child_process');
    const patchExecutor = execSync('ps aux | grep "patch-executor" | grep -v grep | wc -l', { encoding: 'utf8' });
    const orchestrator = execSync('ps aux | grep "orchestrator" | grep -v grep | wc -l', { encoding: 'utf8' });
  ...
  // Check zone status
  Object.entries(PATHS).forEach(([zone, paths]) => {
    const patchCount = fs.readdirSync(paths.patches).filter(f => f.endsWith('.json')).length;
    const summaryCount = fs.readdirSync(paths.summaries).filter(f => f.endsWith('.md')).length;
    
    health.zones[zone] = {
      patches: patchCount,
      summaries: summaryCount,
      lastHeartbeat: fs.existsSync(path.join(paths.heartbeat, '.last-md-write.log'))
    };
  });
...
app.get('/patches/:zone', (req, res) => {
  ...
  try {
    const files = fs.readdirSync(PATHS[zone].patches)
