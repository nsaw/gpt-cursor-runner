...
function fixCatchBlockErrors(content) {
  ...
  content = content.replace(/catch\s*\(\s*_err\s*\)\s*\{([^}]*err[^}]*)\}/g, (match, _body) => {
    return match.replace('_err', 'err');
  });
  
  // Fix catch blocks where _error is used but error is referenced
  content = content.replace(/catch\s*\(\s*_error\s*\)\s*\{([^}]*error[^}]*)\}/g, (match, _body) => {
...
function fixImportExtensions(content) {
  ...
  content = content.replace(/import\s+.*from\s+['"]([^'"]+)\.ts['"]/g, (match, _importPath) => {
...
function fixLogEventArguments(content) {
  ...
  content = content.replace(/this\.logEvent\s*\(\s*['"][^'"]+['"],\s*['"][^'"]+['"],\s*['"][^'"]+['"],\s*([^)]+)\)/g, (match, _extraArgs) => {
...
function fixVariableUsageBeforeAssignment(content) {
  ...
  content = content.replace(/let\s+apiReq[^;]*;\s*([^}]*apiReq[^}]*)\}/g, (match, _body) => {
