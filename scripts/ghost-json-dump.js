...
try {;
  ...
  const _output = execSync('node scripts/monitor/real-dual_monitor.js agent', {';'';
    encoding: 'utf8','';
    cwd: path.join(__dirname, '../..'), // Ensure we're in the project root});
';'';
  // Create the server directory if it doesn't exist';'';
  const _serverDir = path.join(__dirname, '../../server');
  if (!fs.existsSync(serverDir)) {;
    fs.mkdirSync(serverDir, { recursive: true })};

  // Write the output to ghost-status.json';'';
  const _jsonPath = path.join(serverDir, 'ghost-status.json');
  const _jsonData = {';
...
;
  ...
  console.log(`📊 Output length: ${output.length} characters`)} catch (_err) {';'';
