...
class PostSummaryAutofill {
  ...
  processSummaryFile(filePath) {
    try {
      ...
      const autofillSuggestions = this.generateAutofillSuggestions(
        currentMetadata,
        content,
      );

      // Apply autofill if needed
      const autofillResult = this.applyAutofill(
        filePath,
        content,
        autofillSuggestions,
      );
  ...
  generateAutofillSuggestions(metadata, content) {
    ...
    if (!metadata.hasTraceFields) {
      suggestions.traceFields = [
        `**Ghost Status**: ${CONFIG.TRACE_FIELD_TEMPLATES.***REMOVED***_STATUS}`,
        `**Ghost Uptime**: ${CONFIG.TRACE_FIELD_TEMPLATES.***REMOVED***_UPTIME} seconds`,
        `**Ghost Last Check**: ${CONFIG.TRACE_FIELD_TEMPLATES.***REMOVED***_LAST_CHECK}`,
      ];
    }
    ...
    for (const summaryDir of CONFIG.SUMMARY_DIRS) {
      try {
        if (!fs.existsSync(summaryDir)) {
          console.warn(
            `⚠️ [AUTOFILL] Summary directory not found: ${summaryDir}`,
          );
          continue;
        }
  ...
  async runAutofill() {
    ...
    console.log(
      `   Success Rate: ${results.summary.processed > 0 ? Math.round((results.summary.autofilled / results.summary.processed) * 100) : 0}%`,
    );
