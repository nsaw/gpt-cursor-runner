...
function removeFromFailed(base){ const f=p.join(FAILED,base); if(fs.existsSync(f)) try{fs.unlinkSync(f)}catch{} }

(function main(){
  while(true){
    const base=currentCandidate();
    if(!base){ log("NO_CANDIDATE"); process.exit(0); }

    const pAbs = fs.existsSync(p.join(P1,base+".hold")) ? p.join(P1,base+".hold")
              : fs.existsSync(p.join(P1,base)) ? p.join(P1,base) : null;
    if(!pAbs){ log("ERR_NO_SOURCE:"+base); process.exit(2); }
    ...
    for(let attempt=1; attempt<=MAX; attempt++){
      ...
      if(ok){
        ...
        try{ if(fs.existsSync(pAbs)) fs.renameSync(pAbs, p.join(COMPLETED,base)); }catch{}
    ...
    if(!passed){
      ...
      try{ fs.writeFileSync(blocked, JSON.stringify(payload,null,2)); }catch{}
