...
for (const name of order) {
  ...
  if (st.status === 'err')  { log(`RESTORE_ERROR:${name}`); process.exit(2); } // eslint-disable-line no-process-exit
  ...
  if (exists(failPath)) { log(`EXEC_FAIL:${name}`); process.exit(1); } // eslint-disable-line no-process-exit
