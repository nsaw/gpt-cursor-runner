...
const checkPatchSuccess = (patchPath) => {
  ...
  if (name.includes('pm2-nonblocking-wrappers-enforce')) {
    return exists(
      '/Users/sawyer/gitSync/gpt-cursor-runner/scripts/pm2-nonblocking-wrappers.sh',
    );
  }
...
const clearBlockedIfSuccessful = (blockedData) => {
  ...
  if (checkPatchSuccess(patchPath)) {
    console.log(
      `✅ Quick-win detected: ${blockedData.hold} was actually successful`,
    );
...
const manualRepairWithQuickWin = (patchPath, attempt) => {
  ...
  const result = cp.spawnSync(
    'node',
    [
      '/Users/sawyer/gitSync/gpt-cursor-runner/scripts/g2o/manual_repair_playbook_once.js',
      patchPath,
    ],
    {
      stdio: 'inherit',
...
const executePatchWithSuccessCheck = (patchPath) => {
  ...
  const result = cp.spawnSync(
    'node',
    [
      '/Users/sawyer/gitSync/gpt-cursor-runner/scripts/g2o/exec_patch_with_compat_once.js',
      patchPath,
    ],
    {
      stdio: 'inherit',
...
const main = () => {
  ...
  const p1Patches = list(P1)
    .filter(
      (f) =>
        f.endsWith('.json') && !f.endsWith('.hold') && f !== 'P1.BLOCKED.json',
    )
    .sort();
  ...
  cp.spawnSync(
    'node',
    [
      '/Users/sawyer/gitSync/gpt-cursor-runner/scripts/g2o/autofix_forbidden_shell_once.js',
      patchPath,
    ],
    {
      stdio: 'inherit',
