...
function orderList() {
  ...
  let order = list(P1)
    .filter((f) => /^patch-v.+\.json(\.hold)?$/.test(f))
    .map((x) => x.replace(/\.hold$/, ''));
  try {
    const lines = fs
...
function execCompat(abs) {
  return (
    cp.spawnSync(
      'node',
      [p.join(__dirname, 'exec_patch_with_compat_once.js'), abs],
      { stdio: 'inherit' },
    ).status === 0
  );
}
...
(function main() {
  while (true) {
    ...
    for (let i = 1; i <= MAX_SAFE; i++) {
      cp.spawnSync(
        'node',
        [p.join(__dirname, 'autofix_forbidden_shell_once.js'), abs],
        { stdio: 'inherit' },
      );
      if (execCompat(abs)) {
    ...
    for (let a = 1; a <= MAX_MANUAL; a++) {
      const r = cp.spawnSync(
        'node',
        [p.join(__dirname, 'manual_repair_playbook_once.js'), abs],
        { stdio: 'inherit' },
      );
      if (r.status !== 0) {
    ...
    if (!passed) {
      ...
      const obj = {
        ts: Date.now(),
        reason: 'agent_manual_repair_exhausted',
        patch: base,
        attempts: { safe: MAX_SAFE, manual: MAX_MANUAL },
      };
      try {
