...
const P1=p.join(QROOT,"G2o/P1"); const FAILED=p.join(QROOT,".failed"); const COMPLETED=p.join(QROOT,".completed");
const LOGS="/Users/sawyer/gitSync/.cursor-cache/ROOT/.logs"; const TRI="/Users/sawyer/gitSync/.cursor-cache/CYOPS/summaries/_triage";
...
fs.mkdirSync(LOGS,{recursive:true}); fs.mkdirSync(TRI,{recursive:true});
const log=s=>{ fs.appendFileSync(p.join(LOGS,'patch-events.log'),`[SEQ-MANUAL] ${s}\n`); console.log(s); }
...
function restoreFromFailed(base){
  const src=p.join(FAILED,base); if(!fs.existsSync(src)) return false;
  fs.mkdirSync(P1,{recursive:true});
  fs.writeFileSync(p.join(P1,base+".hold"), fs.readFileSync(src)); fs.unlinkSync(src); return true;
}
function candidate(){
...
function execCompat(abs){ return cp.spawnSync("node",[p.join(__dirname,"exec_patch_with_compat_once.js"),abs],{stdio:"inherit"}).status===0; }

(function main(){
  while(true){
    const base=candidate(); if(!base){ log("NO_CANDIDATE"); process.exit(0); }
    const abs = fs.existsSync(p.join(P1,base+".hold")) ? p.join(P1,base+".hold")
              : fs.existsSync(p.join(P1,base)) ? p.join(P1,base) : null;
    if(!abs){ log("ERR_NO_SOURCE:"+base); process.exit(2); }
    ...
    for(let i=1;i<=MAX_SAFE;i++){
      ...
      if(execCompat(abs)){ fs.mkdirSync(COMPLETED,{recursive:true}); try{fs.renameSync(abs,p.join(COMPLETED,base));}catch{} log(`PASS_SAFE:${base}`); continue /* to next patch */; }
      // rehydrate from failed for next step
      const f=p.join(FAILED,base); if(fs.existsSync(f)){ fs.mkdirSync(P1,{recursive:true}); fs.writeFileSync(p.join(P1,base+".hold"), fs.readFileSync(f)); fs.unlinkSync(f); }
    }
    ...
    for(let a=1;a<=MAX_MANUAL;a++){
      ...
      if(r.status!==0){ log(`MANUAL_REPAIR_FAIL_PRE:${base}:attempt=${a}`); continue; }
      if(execCompat(abs)){ fs.mkdirSync(COMPLETED,{recursive:true}); try{fs.renameSync(abs,p.join(COMPLETED,base));}catch{} log(`PASS_MANUAL:${base}:attempt=${a}`); passed=true; break; }
    ...
    if(!passed){
      ...
      try{ fs.writeFileSync(blocked, JSON.stringify(obj,null,2)); }catch{}
