...
function pickGit(){ for (const p of ['/opt/homebrew/bin/git','/usr/local/bin/git','/usr/bin/git','git']) { try { fs.accessSync(p, fs.constants.X_OK); return p; } catch {} } throw new Error('git binary not found'); }
...
(async()=>{try{
  ...
  await run(args,repo); let short=''; try{short=(await run(['rev-parse','--short','HEAD'],repo)).so.trim();}catch{}
  console.log(JSON.stringify({ok:true,commitHash:short,message:message},null,2)); process.exit(0);
