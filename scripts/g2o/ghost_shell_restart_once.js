  ...
  try {
    ...
    exec('pkill -f "ghost.*shell"', (error) => {
      ...
      setTimeout(() => {
        ...
        exec('node /Users/sawyer/gitSync/gpt-cursor-runner/src-nextgen/ghost/shell/index.ts', (error, _stdout, _stderr) => {
          if (error) {
    ...
    console.log('***REMOVED***_SHELL_RESTART_INITIATED');
    process.exit(0);
  } catch (error) {
    console.error(`***REMOVED***_SHELL_RESTART_ERROR:${error.message}`);
    process.exit(1);
  }
}
