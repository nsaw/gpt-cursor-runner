...
function testParseFunctions() {
  ...
  console.log(`✅ Invalid JSON handling: ${!invalidJsonResult ? 'PASS' : 'FAIL'}`);
  console.log(`✅ Invalid YAML handling: ${!invalidYamlResult ? 'PASS' : 'FAIL'}`);
...
function testValidation() {
  ...
  console.log(`✅ Invalid patch (missing role): ${!invalidResult1.valid ? 'PASS' : 'FAIL'}`);
  console.log(`✅ Invalid patch (wrong role): ${!invalidResult2.valid ? 'PASS' : 'FAIL'}`);
  console.log(`✅ Invalid patch (missing target): ${!invalidResult3.valid ? 'PASS' : 'FAIL'}`);
  return (validResult.valid && !invalidResult1.valid && !invalidResult2.valid && !invalidResult3.valid);
...
function testRouting() {
  ...
  console.log(`✅ MAIN routing: ${mainResult.includes('MAIN') ? 'PASS' : 'FAIL'}`);
  console.log(`✅ CYOPS routing: ${cyopsResult.includes('CYOPS') ? 'PASS' : 'FAIL'}`);
  console.log(`✅ DEV routing: ${devResult.includes('CYOPS') ? 'PASS' : 'FAIL'}`);
  console.log(`✅ Unknown routing: ${unknownResult.includes('CYOPS') ? 'PASS' : 'FAIL'}`);
  return (mainResult.includes('MAIN') && cyopsResult.includes('CYOPS') && devResult.includes('CYOPS') && unknownResult.includes('CYOPS'));
...
function testPatchProcessing() {
  ...
  for (const testCase of testCases) {
    ...
    if (testCase.expected.valid) {
      ...
      if (!result.processed && result.reason.includes(testCase.expected.reason)) {
