...
# CLOUDFLARE TUNNEL CONFIGURATION (canonical)
# Use explicit config file to avoid ID confusion
TUNNEL_ID="f1545c78-1a94-408f-ba6b-9c4223b4c2bf"
TUNNEL_CONFIG="/Users/sawyer/.cloudflared/gpt-cursor-runner-config.yml"
...
switch_tunnel_to_fly() {
    ...
    # Update tunnel config (primary Fly, fallback to local Flask 8787)
    nb "sed -i '' 's|service: http://localhost:8787|service: $FLY_URL|g' '$TUNNEL_CONFIG'" 10
...
switch_tunnel_to_local() {
    ...
    # Update tunnel config to local dashboard (8787) fallback
    nb "sed -i '' 's|service: $FLY_URL|service: http://localhost:8787|g' '$TUNNEL_CONFIG'" 10
...
restart_tunnel() {
    ...
    # Start new tunnel
    { (
        cloudflared tunnel run --config "$TUNNEL_CONFIG" >/dev/null 2>&1 &
        local tunnel_pid=$!
...
start_pm2_services() {
    ...
    # Standardize to the canonical ecosystem file under config/
