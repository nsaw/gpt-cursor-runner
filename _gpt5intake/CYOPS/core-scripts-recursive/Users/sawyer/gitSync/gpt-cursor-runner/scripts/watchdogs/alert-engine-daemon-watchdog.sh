...
# Public endpoint health config
MONITOR_URL="https://gpt-cursor-runner.thoughtmarks.app/monitor"
API_STATUS_URL="https://gpt-cursor-runner.thoughtmarks.app/api/status"
SLACK_HEALTH_URL="https://slack.thoughtmarks.app/health"
PUBLIC_ALERT_LOG="$LOG_DIR/public-health-alerts.log"
MAX_FAILS_PER_CYCLE=0

# Curl-based checker (200 OK expected)
check_public_url() {
    local url="$1"
    local label="$2"
    local code
    ...
    if [ "$code" = "200" ]; then
        log "[PUBLIC-OK] $label ($url) -> $code"
        return 0
    else
        log "[PUBLIC-FAIL] $label ($url) -> $code" "error"
        echo "$(date -u +%FT%TZ) [PUBLIC-FAIL] $label $url -> $code" >> "$PUBLIC_ALERT_LOG"
        return 1
    fi
}
...
monitor() {
    ...
    while true; do
        if ! is_daemon_running; then
            ...
            # Public health checks (non-blocking awareness)
            local fails=0
            check_public_url "$MONITOR_URL" "Monitor" || fails=$((fails+1))
            check_public_url "$API_STATUS_URL" "API Status" || fails=$((fails+1))
            check_public_url "$SLACK_HEALTH_URL" "Slack Health" || fails=$((fails+1))

            if [ "$fails" -gt "$MAX_FAILS_PER_CYCLE" ]; then
                log "Public endpoints degraded (fails=$fails). Alert engine will capture logs; manual check recommended." "error"
            fi
        fi
