#!/bin/bash
# Telemetry Orchestrator Daemon Watchdog
# Monitors and restarts the telemetry orchestrator daemon if needed
...
# Configuration
DAEMON_NAME="telemetry-orchestrator-daemon"
...
start_daemon() {
    ...
    { node "$DAEMON_SCRIPT" > "$LOG_DIR/${DAEMON_NAME}.log" 2>&1 & } >/dev/null 2>&1 & disown
    local new_pid="$(pgrep -f "$DAEMON_NAME" | head -1)"
