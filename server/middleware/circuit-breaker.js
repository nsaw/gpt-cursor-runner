...
const getBreakerStatus = () => {
  return {
    ...
    slack: {
      state: slackBreaker.opened
        ? "open"
        : slackBreaker.halfOpen
          ? "half-open"
          : "closed",
      stats: slackBreaker.stats,
    },
    database: {
      state: databaseBreaker.opened
        ? "open"
        : databaseBreaker.halfOpen
          ? "half-open"
          : "closed",
      stats: databaseBreaker.stats,
    },
    webhook: {
      state: webhookBreaker.opened
        ? "open"
        : webhookBreaker.halfOpen
          ? "half-open"
          : "closed",
...
[redisBreaker, slackBreaker, databaseBreaker, webhookBreaker].forEach(
  (breaker) => {
    breaker.on("open", () => {
    ...
    breaker.on("fallback", (result) => {
      console.log(
        `Circuit breaker ${breaker.name} fallback triggered:`,
        result,
      );
    });
  },
