...
class MetricsTracker:
            ...
            try:
                if slack_proxy:
                    slack_proxy.notify_error(
                        f"Error writing metrics: {e}", context=self.metrics_file
                    )
            except Exception:
                pass
    ...
    def _read_metrics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                if slack_proxy:
                    slack_proxy.notify_error(
                        f"Error reading metrics: {e}", context=self.metrics_file
                    )
            except Exception:
                pass
            return {
    ...
    def count_matches(self, content: str, pattern: str) -> int:
        ...
        try:
            ...
            try:
                if slack_proxy:
                    slack_proxy.notify_error(
                        f"Error in count_matches: {e}", context=pattern
                    )
            except Exception:
                pass
            return 0
    ...
    def end_patch_tracking(
        ...
        metrics = PatchMetrics(
            ...
            replacements_made=self.count_matches(content_before, pattern)
            - self.count_matches(content_after, pattern),
    ...
    def _update_summary(self, data: Dict[str, Any]):
        ...
        summary = {
            ...
            "average_duration_ms": sum(p.get("duration_ms", 0) for p in patches)
            / len(patches),
            "total_matches": sum(p.get("matches_found", 0) for p in patches),
            "average_complexity": sum(p.get("pattern_complexity", 0) for p in patches)
            / len(patches),
            "total_replacements": sum(p.get("replacements_made", 0) for p in patches),
            "average_size_change_bytes": sum(
