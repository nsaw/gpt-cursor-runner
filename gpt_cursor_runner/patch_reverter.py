...
class PatchReverter:
    ...
    def revert_by_patch_id(
        self, patch_id: str, target_file: str = None
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            current_backup = f"{target_file}.revert_backup_{
            ...
            result.update(
                {
                    "success": True,
                    "message": f"Successfully reverted {target_file} to state from {
        ...
        for patch_file in patch_files:
            try:
                ...
                try:
                    ...
                    context_str = (
                        patch_file if isinstance(patch_file, str) else str(patch_file)
                    )
                    slack_proxy.notify_error(
                        f"Error reading patch file in _find_patch_by_id: {e}",
                        context=context_str,
                    )
                except Exception:
                    pass
        ...
        for backup_file in backup_files:
            try:
                ...
                try:
                    ...
                    slack_proxy.notify_error(
                        f"Error processing backup file in cleanup_old_backups: {e}",
                        context=context_str,
                    )
                except Exception:
                    pass
