...
class UnifiedProcessor:
        ...
        if not self.workers:
            ...
            for i in range(self.max_workers):
                worker = threading.Thread(
                    target=self._worker_loop, daemon=True, name=f"processor-{i}"
                )
                worker.start()
    ...
    def _process_request(self, request: ProcessingRequest):
        ...
        try:
            ...
            logger.info(
                f"Request {request.request_id} completed in {processing_time:.2f}s"
            )
            ...
            if request.retry_count < request.max_retries:
                ...
                logger.warning(
                    f"Request {request.request_id} failed, retrying ({request.retry_count}/{request.max_retries})"
                )
            else:
                # Final failure
                with self._lock:
                ...
                logger.error(
                    f"Request {request.request_id} failed after {request.max_retries} retries: {error_msg}"
                )
