...
Provides Flask routes for visualizing patch applications, Slack events, and system
metrics.
"""
    ...
    def dashboard_events():
        ...
        try:
            ...
            events = event_logger.get_recent_events(
                limit,
                event_type if isinstance(event_type, str) and event_type else None,
            )
    ...
    if event_logger:
        try:
            ...
            stats["slack"] = {
                ...
                "commands": len(
                    [e for e in slack_events if e.get("subtype") == "slash_command"]
                ),
                "mentions": len(
                    [e for e in slack_events if e.get("subtype") == "app_mention"]
                ),
...
def get_recent_patches(limit: int = 10) -> List[Dict[str, Any]]:
    ...
    for filepath in patch_files[:limit]:
        try:
            ...
            patches.append(
                {"error": f"Error reading {filepath}: {str(e)}", "filepath": filepath}
            )
            try:
                if slack_proxy:
                    slack_proxy.notify_error(
    ...
    try:
        ...
        if event_logger:
            event_logger.log_system_event(
                "dashboard_is_recent_file_error",
                {"error": str(e), "filepath": filepath},
            )
        try:
            if slack_proxy:
                slack_proxy.notify_error(
                    f"Error in is_recent_file: {e}", context=filepath
                )
        except Exception:
            pass
        return False
...
def is_recent_event(event: Dict[str, Any], hours: int) -> bool:
    ...
    try:
        ...
        if event_logger:
            event_logger.log_system_event(
                "dashboard_is_recent_event_error", {"error": str(e), "event": event}
            )
        try:
            if slack_proxy:
                slack_proxy.notify_error(
                    f"Error in is_recent_event: {e}", context=str(event)
                )
        except Exception:
            pass
        return False
...
def get_tunnel_status() -> Dict[str, Any]:
    ...
    try:
            ...
            try:
                ...
                if "ngrok" in name or "ngrok" in cmdline:
                    tunnels["ngrok"]["processes"].append(
                        {"pid": proc_info["pid"], "cmdline": cmdline}
                    )
        ...
        tunnel_configs = {
            ...
            "cloudflared": os.path.exists("cloudflared.yml")
            or os.path.exists(".cloudflared/config.yml"),
        ...
        for proc in psutil.process_iter(["pid", "name", "cmdline"]):
            try:
                ...
                if "python" in name and (
                    "gpt_cursor_runner" in cmdline or "main.py" in cmdline
                ):
        ...
        for proc in psutil.process_iter(["pid", "name", "cmdline"]):
            try:
                ...
                if "python" in proc_info["name"].lower() and (
                    "flask" in cmdline or "dashboard" in cmdline
                ):
        ...
        if event_logger:
            try:
                ...
                return {
                    ...
                    "recent_24h": len(
                        [e for e in slack_events if is_recent_event(e, 24)]
                    ),
                    "recent_7d": len(
                        [e for e in slack_events if is_recent_event(e, 7 * 24)]
                    ),
                }
...
commandsList.innerHTML = '<div class="loading">No Slack command information
available</div>';
                }
            } catch (error) {
