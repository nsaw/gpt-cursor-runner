...
const DEV_TUNNEL = "https://gpt-cursor-runner.thoughtmarks.app";
const PROD_TUNNEL = "https://gpt-cursor-runner.thoughtmarks.app";
const fallbackRunnerURL = "https://gpt-cursor-runner.thoughtmarks.app";
...
if (fs.existsSync(runnerEnvPath)) {
  const env = fs.readFileSync(runnerEnvPath, "utf8");
  const regex = /^PUBLIC_RUNNER_URL=.*$/m;
  const replacement = `PUBLIC_RUNNER_URL=${resolvedURL}`;
  if (regex.test(env)) {
    const updatedEnv = env.replace(regex, replacement);
    fs.writeFileSync(runnerEnvPath, updatedEnv);
  } else {
