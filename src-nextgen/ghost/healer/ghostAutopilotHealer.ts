...
const healerStatePath =
  "/Users/sawyer/gitSync/.cursor-cache/CYOPS/healer/healer-state.json";
const configPath =
...
class GhostAutopilotHealer {
  ...
  private isRestartLimitExceeded(componentId: string): boolean {
    ...
    if (
      this.lastGlobalRestart > oneHourAgo &&
      this.globalRestartCount >= this.config.safety.maxRestartsPerHour
    ) {
      return true;
    }
  ...
  private shouldInhibitHealing(
    ...
    if (
      this.config.inhibition.configBasedFailures &&
    ...
    if (
      (this.config.inhibition.resourceExhaustion &&
    ...
    if (
      (this.config.inhibition.externalDependencies &&
  ...
  private async executeHealingAction(action: HealingAction): Promise<boolean> {
    ...
    try {
      console.log(
        `[GhostAutopilotHealer] Executing healing action: ${action.action} for ${action.component}`,
      );
  ...
  private async restartComponent(componentId: string): Promise<void> {
    ...
    // Restart component
    await execAsync(
      `node src-nextgen/ghost/${this.getComponentPath(componentId)} &`,
    );

    // Update health metrics
    health.restartCount++;
    health.lastRestart = new Date().toISOString();
    health.cooldownUntil = new Date(
      Date.now() + this.config.safety.cooldownPeriodMs,
    ).toISOString();
  ...
  private getComponentPath(componentId: string): string {
    const pathMap: { [key: string]: string } = {
      ...
      configurationValidationEngine:
        "validation/configurationValidationEngine.ts",
    ...
    try {
      ...
      for (const componentId of componentIds) {
        ...
        if (health.status === "failed" || health.status === "critical") {
          ...
          if (
            this.shouldInhibitHealing(componentId, "Component failure detected")
          ) {
            this.metrics.inhibitedActions++;
            console.log(
              `[GhostAutopilotHealer] Healing inhibited for ${componentId}: ${health.inhibitionReason}`,
            );
            continue;
          }
          ...
          if (this.isInCooldown(componentId)) {
            console.log(
              `[GhostAutopilotHealer] ${componentId} in cooldown period`,
            );
            continue;
          }
          ...
          if (this.isRestartLimitExceeded(componentId)) {
            console.log(
              `[GhostAutopilotHealer] Restart limit exceeded for ${componentId}`,
            );
            continue;
          }
      ...
      this.metrics.componentsInhibited = Array.from(
        this.componentHealth.values(),
      ).filter((h) => h.inhibited).length;
    } catch (error) {
      console.error(
        "[GhostAutopilotHealer] Error in analyze and heal cycle:",
        error,
      );
    }
  }
  ...
  public isHealthy(): boolean {
    return (
      this.isRunning &&
      this.metrics.failedActions / Math.max(this.metrics.totalActions, 1) < 0.2
    );
  }
}
