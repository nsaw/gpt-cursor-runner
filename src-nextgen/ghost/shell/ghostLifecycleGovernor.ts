...
const governorLogPath =
  "/Users/sawyer/gitSync/.cursor-cache/CYOPS/logs/lifecycle-governor.log";
...
function buildStartupQueue(): string[] {
  ...
  const sortedConfigs = [...daemonConfigs].sort(
...
async function governLifecycle(): Promise<void> {
  try {
    ...
    if (
      !lifecycleState.isInitializing &&
      lifecycleState.startupQueue.length > 0
    ) {
