// ***REMOVED*** Alert Engine — Phase 8A P8.08.00
...
const alertLogPath =
  "/Users/sawyer/gitSync/.cursor-cache/CYOPS/logs/alert-engine.log";
const alertStatePath =
  "/Users/sawyer/gitSync/.cursor-cache/CYOPS/telemetry/alert-engine-state.json";
const configPath =
  "/Users/sawyer/gitSync/.cursor-cache/CYOPS/config/alert-engine-config.json";
...
class GhostAlertEngine {
    ...
    try {
      ...
      if (rule.lastTriggered) {
        ...
        const timeSinceLastTrigger =
          (now.getTime() - lastTriggered.getTime()) / 1000;
  ...
  private async evaluateCondition(condition: AlertCondition): Promise<boolean> {
    try {
      ...
      // Apply aggregation
      const aggregatedValue = this.aggregateMetricData(
        metricData,
  ...
  private async sendNotification(
    ...
    if (this.config.notifications.rateLimiting) {
      const now = new Date();
      const lastUsed = channel.lastUsed
  ...
  private async sendWebhookNotification(
    ...
    try {
      ...
      const payload = {
        ...
        username: channel.config.username || "***REMOVED*** Alert Engine",
        timestamp: new Date().toISOString(),
      };
  ...
  private async sendEmailNotification(
    ...
    try {
      ...
      const emailContent = `
Subject: ***REMOVED*** Alert Engine - ${alertEvent.severity.toUpperCase()}: ${alertEvent.ruleName}
...
---
This is an automated alert from the ***REMOVED*** Alert Engine.
Please check the dashboard for more details: https://gpt-cursor-runner.thoughtmarks.app/monitor
      `;

      const { stdout, stderr } = await execAsync(
        `echo "${emailContent}" | mail -s "***REMOVED*** Alert: ${alertEvent.ruleName}" ${config.to}`,
      );
      ...
      const automationStrategies = {
        "daemon-failure": async () => {
          ...
          if (!stdout.trim()) {
            ...
            await execAsync(
              "bash scripts/watchdogs/alert-engine-daemon-watchdog.sh",
            );
          }
        },
  ...
  private async cleanupResolvedAlerts(): Promise<void> {
    ...
    if (this.state.alertHistory.length > this.maxAlertHistory) {
      this.state.alertHistory = this.state.alertHistory.slice(
