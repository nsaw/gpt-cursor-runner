// ***REMOVED*** Metrics Aggregator — Phase 8A P8.07.00
...
const aggregatorLogPath =
  "/Users/sawyer/gitSync/.cursor-cache/CYOPS/logs/metrics-aggregator.log";
const aggregatorStatePath =
  "/Users/sawyer/gitSync/.cursor-cache/CYOPS/telemetry/metrics-aggregator-state.json";
const configPath =
  "/Users/sawyer/gitSync/.cursor-cache/CYOPS/config/metrics-aggregator-config.json";
const analyticsPath =
...
class GhostMetricsAggregator {
  ...
  private async collectMetricsFromSource(
    ...
    try {
      ...
      this.logEvent(
        "collection_error",
        `Failed to collect metrics from ${source.name}: ${error}`,
        "error",
      );
    }
  ...
  private extractHeartbeatMetrics(
    ...
    if (data.heartbeatStatus) {
      ...
      metrics.push({
        ...
        metadata: {
          systemTime: hs.systemTime,
          lastHeartbeat: hs.lastHeartbeat,
        },
      });
      ...
      metrics.push({
        ...
        name: "daemon_health_ratio",
        value:
          hs.daemonCount > 0 ? (hs.healthyDaemons / hs.daemonCount) * 100 : 0,
        unit: "percent",
        source: sourceId,
  ...
  private async collectAllMetrics(): Promise<void> {
    ...
    if (this.state.aggregatedMetrics.length > this.maxMetricHistory) {
      this.state.aggregatedMetrics = this.state.aggregatedMetrics.slice(
    ...
    for (const [metricName, metrics] of Object.entries(metricGroups)) {
      if (metrics.length >= 20) {
        ...
        if (baseline) {
          ...
          const deviation =
            Math.abs(avgValue - baseline.baselineValue) /
  ...
  private getBaseline(metricName: string): PerformanceBaseline | null {
    return (
      this.state.baselines.find((b) => b.metricName === metricName) || null
    );
  }
  ...
  private async calculateHealthScore(): Promise<void> {
    ...
    // Performance factors
    const performanceMetrics = this.state.aggregatedMetrics.filter(
    ...
    const avgResponseTime =
      ...
      Math.max(
        1,
        performanceMetrics.filter((m) => m.name.includes("response_time"))
          .length,
      );
    ...
    // Reliability factors
    const reliabilityMetrics = this.state.aggregatedMetrics.filter(
    ...
    const avgHealthRatio =
      ...
      Math.max(
        1,
        reliabilityMetrics.filter((m) => m.name.includes("health_ratio"))
          .length,
      );
    ...
    const activeSources = this.state.sources.filter(
      (s) => s.status === "active",
    ).length;
    ...
    while (this.isRunning) {
      try {
        ...
        await new Promise((resolve) =>
          setTimeout(resolve, this.config.collection.intervalMs),
        );
      } catch (error) {
        this.logEvent(
          "aggregation_error",
