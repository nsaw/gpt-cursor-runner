...
const dashboardLogPath = '/Users/sawyer/gitSync/.cursor-cache/CYOPS/logs/monitor-dashboard.log';
const dashboardStatePath = '/Users/sawyer/gitSync/.cursor-cache/CYOPS/telemetry/dashboard-state.json';
const configPath = '/Users/sawyer/gitSync/.cursor-cache/CYOPS/config/environment-config.json';
...
class MonitorDashboard {
  ...
  private async fetchTelemetryData(endpoint: string): Promise<any> {
    try {
      const data = await fetchWithTimeout(`${this.config.telemetryApiUrl}${endpoint}`, {}, 5000);
      if (data) {
      ...
      switch (panel.type) {
        case 'health':
          if (panel.id === 'system-health') {
            const healthData = await this.fetchTelemetryData('/api/telemetry/health');
            if (healthData) {
              panel.data = healthData;
            }
          } else if (panel.id === 'component-status') {
            const componentData = await this.fetchTelemetryData('/api/telemetry/components');
            if (componentData) {
        ...
        case 'metrics':
          if (panel.id === 'telemetry-metrics') {
            const metricsData = await this.fetchTelemetryData('/api/telemetry/metrics');
            if (metricsData) {
              panel.data = metricsData;
            }
          } else if (panel.id === 'heartbeat-monitor') {
            const heartbeatData = await this.fetchTelemetryData('/api/telemetry/metrics/heartbeat');
            if (heartbeatData) {
              panel.data = heartbeatData;
            }
          } else if (panel.id === 'performance-audit') {
            const auditData = await this.fetchTelemetryData('/api/telemetry/metrics/loopAuditor');
            if (auditData) {
        ...
        case 'alerts':
          const alertsData = await this.fetchTelemetryData('/api/telemetry/alerts?status=active&limit=10');
          if (alertsData) {
        ...
        case 'logs':
          ...
          const logsData = await this.fetchTelemetryData('/api/telemetry/events?eventType=log&limit=20');
          if (logsData) {
        ...
        case 'events':
          if (panel.id === 'system-snapshots') {
            const snapshotData = await this.fetchTelemetryData('/api/telemetry/events?eventType=snapshot&limit=5');
            if (snapshotData) {
  ...
  private async updateAllPanels(): Promise<void> {
    const updatePromises = this.panels.map(panel => this.updatePanelData(panel));
    ...
    const healthyPanels = this.panels.filter(p => p.data && Object.keys(p.data).length > 0).length;
...
export async function fetchTelemetryWithFallback(endpoint: string, fallback: any = null, timeoutMs = 5000) {
  try {
    const data = await fetchWithTimeout(`/api/telemetry/${endpoint}`, {}, timeoutMs);
